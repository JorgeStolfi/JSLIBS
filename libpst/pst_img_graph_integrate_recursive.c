/* See {pst_img_graph.h} */
/* Last edited on 2024-12-23 06:49:37 by stolfi */
            ...
            if (v->mark == MARK_VERTEX_NONE)
              { stats[deg-1] = stats[deg-1] + 1;
                ...
                if ( v->edge != oct_arc_NULL)
                  { oct_arc_t e = v->edge;
                    do 
                      { uint32_t dest = pst_img_graph_get_edge_origin(g,oct_sym(e));
                        if (dest == -1)
  ...
  { assert(n_neighbours >=3);
    ...
    do
      { oct_arc_t f = oct_onext(e);
        ...
        w = wmag*w;
        if (w != 0)
          { uint32_t ve = pst_img_graph_get_edge_origin(g,oct_sym(e));
            uint32_t vf = pst_img_graph_get_edge_origin(g,oct_sym(f));
  ...
  { uint32_t n_neighbours = pst_img_graph_vertex_count_neighbours(g,vi);
    ...
    if (n_neighbours == 0)
      ...
      { oct_arc_t e1 = oct_onext(e0);
        ...
        uint32_t v0 = pst_img_graph_get_edge_origin(g,oct_sym(e0));
        uint32_t v1 = pst_img_graph_get_edge_origin(g,oct_sym(e1));
...
void pst_img_graph_shrink(pst_img_graph_t *g,double *w_i,double wmag)
  {
    ...
    uint32_t *degree = (uint32_t*)notnull(malloc(sizeof(uint32_t)*(g->n)),"no mem");
    for (uint32_t i = 0; i < g->n; i++)
      ...
      { pst_vertex_data_t *vIG = &(ig->vertex[i]);
        ...
        if (vIG->mark == MARK_VERTEX_REMOVED)
          { oct_arc_t e0 = vIG->edge;
            ...
            double sWZ = 0;
            do
              { uint32_t ind_dst = pst_img_graph_get_edge_origin(ig,oct_sym(e));
...
/* pst_img_graph_integration_recursive(jg,jZ,jW,wmag,(uint32_t)ceil(maxIter*M_SQRT2),convTol/M_SQRT2,para,szero,verbose,level+1,OZ,RZ,out_prefix,debug); */
    ...
    for (uint32_t i = 0; i < g->n;i++) iZ[i] = 0; 
  }
  ...
  int32_t *ref_tab;
  pst_imgsys_t *S = pst_img_graph_build_integration_system(g, iW, (uint32_t)OZ->sz[1], (uint32_t)OZ->sz[2], &ref_tab);
