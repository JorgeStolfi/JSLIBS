...
uint32_t pst_fit_sphere_num_params(double ctrAdj, double radAdj, double resAdj);
...
uint32_t pst_fit_sphere_num_params(double KAdj, double RAdj, double GAdj)
  {
    uint32_t NP = (uint32_t)((KAdj > 0 ? 2 : 0) + (RAdj > 0 ? 1 : 0) + (GAdj > 0 ? 1 : 0));
    return NP;
  }
