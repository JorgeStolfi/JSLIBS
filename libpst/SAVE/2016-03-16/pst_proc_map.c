/* See pst_proc_map.h */
/* Last edited on 2016-03-16 16:05:13 by stolfilocal */
...
void pst_proc_map_function_21(r2_t p, double *z, r2_t *dz)
  { /* Circular platforms with smooth edges. */
    ...
    double Z[3] = { 0.80, 0.60, 0.70 };   /* Heights. */
    int k;
    (*z) = 0;
    if (dz != NULL) { (*dz) = (r2_t){{0,0}}; }
    for (k = 0; k < 3; k++)
      { 
        r2_t pk; r2_sub(&p, &(ctr[k]), &pk);
        double zk;
        r2_t dzk;
        pst_proc_map_function_round_platform(&pk, R[k], HS[k], &zk, (dz == NULL ? NULL : &dzk));
        (*z) += Z[k]*zk;
        if (dz != NULL) 
          { dz->c[0] += Z[k]*dzk.c[0];
            dz->c[1] += Z[k]*dzk.c[1];
...
void pst_proc_map_function_polygonal_platform(r2_t *p, int N, double R, double tilt, double S, double *z, r2_t *dz)
  {
    ...
    if (r2 >= RO*RO)
    ...
    else
      {
        ...
        if (u <= 0)
          { fprintf(stderr, "p = ( %+8.5f %+8.5f )  ang = %+8.2f  nw = %+2d  t = %+8.2f  uv = ( %+8.5f %+8.5f )  auv = %+8.2f\n", x, y, ang*180/M_PI, nw, t*180/M_PI, u, v, atan2(v,u)*180/M_PI); }
