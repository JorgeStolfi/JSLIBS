...
void generatePOVMesh(FILE* arq,float_image_t* height_map,  float_image_t* normal_map,float_image_t *img, int nx,int ny,double scale);
...
int main(int argc, char** argv){
	...
	if(argc < 5){
		printf("Program usage:\nplot_heights <height_map> <normal_map> <image_file> <output_file> [scale]\n");
		return 1;
	}
	...
	FILE* output_file = fopen(argv[4],"wt");
	double scale = 1.0;
	if(argc ==6){
		scale = atof(argv[5]);
	}
	assert(output_file != NULL);
	generatePOVMesh(output_file,height_map,normal_map,img, height_map->sz[1] -1,height_map->sz[2]-1,scale);
	fclose(output_file);
