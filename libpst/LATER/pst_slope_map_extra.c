/* Last edited on 2008-11-10 17:32:13 by stolfi */

void pst_slope_map_build_integration_system_2(float_image_t *IDZ, float_image_t *IW, imgsys_t* S)
  {

    /* Get/check the sizes of the slope maps: */
    int NC = IDZ->sz[0]; assert(NC == 2);
    int NX = IDZ->sz[1];
    int NY = IDZ->sz[2];
    ...
    /* Conceptually, we define two quadratic mismatch terms
      ...
      obtained from the slope map {IDZ}, and the difference of the 
      ...
      Note that if the slope map {IDZ} is zero, we can anihilate all
    ...
    for(y = 0; y <= NY; y++)
      { for(x = 0; x <= NX; x++)
          { /* Get the slopes and relative weights of the pixels incident to {(x,y)}: */
            ...
            if ((x > 0) && (y > 0))
              { dxmm = get_sample(IDZ, 0, x-1, y-1);
                dymm = get_sample(IDZ, 1, x-1, y-1);
                wmm = get_sample(IW, 0, x-1, y-1);
                assert(wmm >= 0.0);
              }

            if ((x > 0) && (y < NY))
              { dxmp = get_sample(IDZ, 0, x-1, y);
                dymp = get_sample(IDZ, 1, x-1, y);
                wmp = get_sample(IW, 0, x-1, y);
                assert(wmp >= 0.0);
              }

            if ((x < NX) && (y > 0))
              { dxpm = get_sample(IDZ, 0, x, y-1);
                dypm = get_sample(IDZ, 1, x, y-1);
                wpm = get_sample(IW, 0, x, y-1);
                assert(wpm >= 0.0);
              }

            if ((x < NX) && (y < NY))
              { dxpp = get_sample(IDZ, 0, x, y);
                dypp = get_sample(IDZ, 1, x, y);
            ...
            if (wmm != 0.0) 
              { eqk->ix[nt] = kmm; eqk->cf[nt] = -wmm/wtot; eqk->rhs += -wdmm/wtot; nt++; }
            if (wpm != 0.0) 
              { eqk->ix[nt] = kpm; eqk->cf[nt] = -wpm/wtot; eqk->rhs += -wdpm/wtot; nt++; }
            if (wmp != 0.0) 
              { eqk->ix[nt] = kmp; eqk->cf[nt] = -wmp/wtot; eqk->rhs += -wdmp/wtot; nt++; }
            if (wpp != 0.0) 
              { eqk->ix[nt] = kpp; eqk->cf[nt] = -wpp/wtot; eqk->rhs += -wdpp/wtot; nt++; }
            eqk->nt = nt;
          }
      }
  }
