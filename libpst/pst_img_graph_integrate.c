/* See {pst_img_graph.h} */
/* Last edited on 2024-12-23 06:27:50 by stolfi */
...
pst_imgsys_t *pst_img_graph_build_integration_system
  ...
  { /* fprintf(stderr,"Oi\n"); */
    int32_t *ind_ix = talloc(g->n, int32_t);
    pst_imgsys_equation_t *eq = talloc(g->n, pst_imgsys_equation_t);
    uint32_t N = 0;
...
void pst_img_graph_solve_system
  ...
  { if ( g->n_valid == S->N )
    ...
    uint32_t *queue = pst_img_graph_sort_equations(S,ref_tab,iW,g->n );
...
void pst_img_graph_put_solution_into_image(pst_img_graph_t *g, double *iZ, float_image_t *OZ)
  {
    ...
    for (uint32_t i = 0; i < g->n; i++)
      { pst_vertex_data_t *v = &(g->vertex[i]);
        ...
        double z = iZ[i];
        int32_t x,y;
        pst_img_graph_get_vertex_image_indices(&(v->coords), (uint32_t)OZ->sz[1], (uint32_t)OZ->sz[2], &x, &y);
