/* See pst_lamp.h */
/* Last edited on 2024-12-22 22:29:04 by stolfi */
...
void pst_light_adjust_lamp_radii(pst_lamp_vec_t *lmpv, uint32_t IS, uint32_t NS);
...
void pst_light_ensure_one_lamp(pst_light_t *lht, double cmin, pst_lamp_t **src)
  { pst_lamp_vec_t *lmpv = &(lht->lmpv);
    ...
    if (isrc < 0)
      { pst_lamp_vec_expand(lmpv, (vec_index_t)NS);
        r3_t def = (r3_t){{ 0,0,1 }};
...
void pst_light_add_single(pst_lamp_vec_t *lmpv, uint32_t *NSP, double crad)
  { int32_t ks = (int32_t)(*NSP);
    pst_lamp_vec_expand(lmpv, ks);
    r3_t one = (r3_t) {{ +1, 0, 0 }};
    r3_t *dir = (crad <= -1.0 ? NULL : &one );
    lmpv->e[ks] = pst_lamp_new(1, dir, 0.0, crad);
    ks++;
              ...
              { int32_t parity = (i0 + i1 + i2) % 2;
                if ((parity == 0) == dual)
                  { /* A corner of the cube: */
                    ...
                    (void)r3_dir(&dir, &dir);
                    pst_lamp_vec_expand(lmpv, ks);
                    lmpv->e[ks] = pst_lamp_new(1, &dir, 0.0, 1.0);
                    ks++;
                  }
              }
          }
...
void pst_light_add_uniform_array
  ...
  { 
    uint32_t IS = (*NSP); /* Save initial number of lamps. */
    switch(NA)
      {
      case 1: 
        /* Single ambient lamp. */
      ...
      { /* Set all power vectors to copies of {pwr}. */
        ...
        for (uint32_t i = IS; i < (*NSP); i++)
          { pst_lamp_t *src = lmpv->e[i];
            ...
            for (uint32_t c = 0; c < KC; c++) { src->pwr.e[c] = pwr->e[c]; }
          }
      }
...
pst_light_t pst_light_spec_parse(argparser_t *pp, bool_t next, uint32_t *NCP)
  { 
    uint32_t NS = 0; /* Number of light sources. */
