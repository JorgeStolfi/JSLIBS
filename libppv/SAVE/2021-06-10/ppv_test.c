/* Last edited on 2021-06-10 15:45:45 by jstolfi */ 
...
#define bug2(MSG,TAG1,FMT1,ARG1,TAG2,FMT2,ARG2) \
  do { \
    fprintf(stderr, "%s:%d: ", __FILE__, __LINE__); \
    fputs(MSG, stderr); \
    fputs(TAG1, stderr); \
...
void test_new_array(ppv_array_t *A, ppv_size_t *sz, ppv_nbits_t bps, ppv_nbits_t bpw)
  {
    ...
    if (N != 6) { bug0("unexpected num of axes - fix test"); }

    fprintf(stderr, "Checking ppv_new_array, ppv_descriptor_is_valid...\n");
    bool_t verbose = TRUE;
    if (verbose) { ppv_print_descriptor(stderr, "A = { ", A, " }\n"); } 
    if (! ppv_descriptor_is_valid(A, TRUE)) { bug0("ppv_new_array returns invalid desc"); }
    if (A->base != 0) { bug1("", "base = ", ppv_pos_t_FMT, A->base); }
    if (A->bps != bps) { bug1("", "bpw = ", "%u", A->bps); }
    if (A->bpw != bpw) { bug1("", "bpw = ", "%u", A->bpw); }
    /* Check steps for a vanilla array: */
    ppv_sample_count_t stp = 1;
    { ppv_axis_t i;
      for (i = 0; i < N; i++) 
        { if (A->step[i] != stp) { bug2("", "step", "[%d] = ", i, "", ppv_step_t_FMT, A->step[i]); }
          stp *= sz[i];
        }
    }
...
void test_packing(ppv_array_t *A)
  {
    ...
    for (pos = 0; pos < npos; pos++)
      { ppv_sample_t xsmp = (ppv_sample_t)((smask & (2*pos)) | 1 | (1LU <<(A->bps-1)));
        if (verbose) { fprintf(stderr, "setting sample " ppv_pos_t_FMT " to %u\n", pos, xsmp); }
        ...
        if (smp != xsmp) 
          { bug2("set/get sample mismatch", " smp = ", ppv_sample_t_FMT, smp, " xsmp = ", ppv_sample_t_FMT, xsmp); } 
      }
    fprintf(stderr, "\n");
  }
            ...
            for (ixA[1] = 0; ixA[1] < A->size[1]; ixA[1]++)
              for (ixA[0] = 0; ixA[0] < A->size[0]; ixA[0]++)
                { 
                  ...
                  if (pos != xpos) 
                    { bug2("ppv_sample_pos mismatch", " pos = ", ppv_pos_t_FMT, pos, " xpos = ", ppv_pos_t_FMT, xpos); } 

                  ppv_sample_t smp = (ppv_sample_t)(smask & (~xpos));
                  ppv_set_sample_at_pos(A->el, A->bps, A->bpw, pos, smp);
                  ppv_sample_t xsmp = ppv_get_sample_at_pos(A->el, A->bps, A->bpw, pos);
                  if (xsmp != smp) 
                    { bug2("set/get_at_pos sample mismatch", " smp = ", ppv_sample_t_FMT, smp, " xsmp = ", ppv_sample_t_FMT, xsmp); } 

                  smp = (ppv_sample_t)(smask & xpos);
                  ppv_set_sample(A,ixA, smp);
                  ppv_sample_t ysmp = ppv_get_sample(A, ixA);
                  if (ysmp != smp) 
                    { bug2("set/get sample mismatch", " smp = ", ppv_sample_t_FMT, smp, " ysmp = ", ppv_sample_t_FMT, ysmp); } 
          ...
          for (ixA[2] = 0; ixA[2] < A->size[2]; ixA[2]++)
            for (ixA[1] = 0; ixA[1] < A->size[1]; ixA[1]++)
              for (ixA[0] = 0; ixA[0] < A->size[0]; ixA[0]++)
                { 
                  ppv_pos_t pos = ppv_sample_pos(A, ixA);
                  ppv_sample_t xsmp = (ppv_sample_t)(smask & pos);
                  ppv_sample_t smp = ppv_get_sample_at_pos(A->el, A->bps, A->bpw, pos);
                  if (smp != xsmp) 
                    { bug2("set/get sample mismatch", " smp = ", ppv_sample_t_FMT, smp, " xsmp = ", ppv_sample_t_FMT, xsmp); } 
                }
...
void test_crop(ppv_array_t *A)
  {
    ...
    for (trial = 0; trial < ntrials; trial++)
      { /* Initialze {sh} and {sz} for full array: */
        ...
        ppv_axis_t i1 = (ppv_axis_t)(trial % N);
        ...
        if (verbose) 
          { fprintf(stderr, "ppv_crop(%d, " ppv_size_t_FMT " " ppv_size_t_FMT ")", i1, skip1, keep1);
        ...
        ppv_axis_t i2 = (ppv_axis_t)((trial / N) % N);
        ...
        if (verbose) 
          { fprintf(stderr, "ppv_crop(%d, " ppv_size_t_FMT ", " ppv_size_t_FMT ")", i2, skip2, keep2);
            ppv_print_descriptor(stderr, " = { ", &B, " }\n");
          } 
        /* Check validity of descriptor: */
        if (! ppv_descriptor_is_valid(&B, TRUE)) { bug0("ppv_crop returns an invalid desc"); }
                ...
                for (ixB[1] = 0; ixB[1] < sz[1]; ixB[1]++)
                  for (ixB[0] = 0; ixB[0] < sz[0]; ixB[0]++)
                    { 
                      ...
                      if (posB != posA)
                        { bug2("ppv_crop position error", " posB = ", ppv_pos_t_FMT, posB, " posA = ", ppv_pos_t_FMT, posA); } 
                    }
      }
  }
...
void test_subsample(ppv_array_t *A) 
  {
    ...
    for (trial = 0; trial < ntrials; trial++)
      { /* Initialze {st} and {sz} for full array: */
        ...
        if (verbose) 
          { fprintf(stderr, "ppv_subsample(%d, " ppv_size_t_FMT ")", i1, step1);
            ppv_print_descriptor(stderr, " = { ", &B, " }\n");
          } 
        /* Check validity of descriptor: */
        ...
        if (verbose) 
          { fprintf(stderr, "ppv_subsample(%d, " ppv_size_t_FMT ")", i2, step2);
            ppv_print_descriptor(stderr, " = { ", &B, " }\n");
          } 
        /* Check validity of descriptor: */
        if (! ppv_descriptor_is_valid(&B, TRUE)) { bug0("ppv_subsample returns an invalid desc"); }
                ...
                for (ixB[1] = 0; ixB[1] < sz[1]; ixB[1]++)
                  for (ixB[0] = 0; ixB[0] < sz[0]; ixB[0]++)
                    { 
                      ...
                      ppv_pos_t posA = ppv_sample_pos(A, ixA);
                      if (posB != posA)
                        { bug2("ppv_subsample position error", " posB = ", ppv_pos_t_FMT, posB, " posA = ", ppv_pos_t_FMT, posA); } 
                    }
      }
  }
      ...
      { /* Initialze {tr} with identity permutation: */
        ...
        for (pass = 0; pass < 2; pass++)
          { /* Perform a transposition: */
            ppv_axis_t i1 = (ppv_axis_t)((trial/(1+pass)) % N);
            ppv_axis_t j1 = (ppv_axis_t)((trial/(2+pass)) % N);
            ...
            { int k;
              for (k = 0; k < n1; k++)
                { ppv_size_t tmp = tr[i1+k]; tr[i1+k] = tr[j1+k]; tr[j1+k] = tmp; } 
            }
          ...
          for (i = 0; i < N; i++)
            { /* Check whether the size of {B} is correct: */
              if (B.size[i] != A->size[tr[i]]) 
                { bug2("", "size", "[%d] = ", i, "", ppv_size_t_FMT, B.size[i]); }
              /* Check whether the increments of {B} are correct: */
              if (B.step[i] != A->step[tr[i]]) 
                { bug2("", "step", "[%d] = ", i, "", ppv_step_t_FMT, B.step[i]); }
            }
        }
        /* Now check coincidence of the two arrays: */
        ppv_index_t ixA[N], ixB[N];
                ...
                for (ixA[1] = 0; ixA[1] < A->size[1]; ixA[1]++)
                  for (ixA[0] = 0; ixA[0] < A->size[0]; ixA[0]++)
                    { 
                      ...
                      ppv_pos_t posA = ppv_sample_pos(A, ixA);
                      if (posB != posA)
                        { bug2("ppv_swap_indices pos error", " posB = ", ppv_pos_t_FMT, posB, " posA = ", ppv_pos_t_FMT, posA); } 
                    }
      }
  }
...
void test_flip_indices(ppv_array_t *A)
  {
    ...
    for (trial = 0; trial < ntrials; trial++)
      { /* Initialze {tr} with identity permutation: */
        ...
        ppv_axis_t i1 = (ppv_axis_t)(trial % N);
        ppv_axis_t j1 = (ppv_axis_t)(i1 + (trial/2 % (N-i1)));
        { ppv_axis_t ia=i1, ja=j1;
          while (ia < ja) 
            { ppv_size_t tmp = tr[ia]; tr[ia] = tr[ja]; tr[ja] = tmp; ia++; ja--; }
        }
        ...
        ppv_axis_t i2 = (ppv_axis_t)((trial/N) % N);
        ppv_axis_t j2 = (ppv_axis_t)(i2 + ((trial/N/2) % (N-i2)));
                ...
                for (ixA[1] = 0; ixA[1] < A->size[1]; ixA[1]++)
                  for (ixA[0] = 0; ixA[0] < A->size[0]; ixA[0]++)
                    { 
                      ...
                      ppv_pos_t posA = ppv_sample_pos(A, ixA);
                      if (posB != posA)
                        { bug2("ppv_flip_indices pos error", " posB = ", ppv_pos_t_FMT, posB, " posA = ", ppv_pos_t_FMT, posA); } 
                    }
      }
  }
...
void test_flip(ppv_array_t *A)
  {
    ...
    for (trial = 0; trial < ntrials; trial++)
      { /* Initialze {fp} for unflipped array: */
        ...
        /* Check validity of descriptor: */
        if (! ppv_descriptor_is_valid(&B, TRUE)) { bug0("ppv_flip returns an invalid desc"); }
        ...
        /* Check consistency of storage area: */
        if (B.el != A->el) { bug0("ppv_flip garbled storage area"); }
        /* Check whether the size of {B} is correct: */
                ...
                for (ixB[1] = 0; ixB[1] < B.size[1]; ixB[1]++)
                  for (ixB[0] = 0; ixB[0] < B.size[0]; ixB[0]++)
                    { 
                      ...
                      ppv_pos_t posA = ppv_sample_pos(A, ixA);
                      if (posB != posA)
                        { bug2("ppv_flip position error", " posB = ", ppv_pos_t_FMT, posB, " posA = ", ppv_pos_t_FMT, posA); } 
                    }
      }
  }
...
void test_diagonal(ppv_array_t *A)
  {
    ...
    for (trial = 0; trial < ntrials; trial++)
      { /* Initialze {sz} for full array: */
        ...
        ppv_axis_t i1 = (ppv_axis_t)(trial % N);
        ppv_axis_t j1 = (ppv_axis_t)(trial/N % (N-1));
        if (j1 >= i1) { j1++; }
                    ...
                    for (ixB[1] = 0; ixB[1] < B.size[1]; ixB[1]++)
                      for (ixB[0] = 0; ixB[0] < B.size[0]; ixB[0]++)
                        { 
                          ...
                          ppv_pos_t posA = ppv_sample_pos(A, ixA);
                          if (posB != posA)
                            { bug2("ppv_diagonal position error", " posB = ", ppv_pos_t_FMT, posB, " posA = ", ppv_pos_t_FMT, posA); } 
                        }
          }
      }
  }
...
void test_chop(ppv_array_t *A)
  {
    ...
    for (trial = 0; trial < ntrials; trial++)
      { /* Initialze {sz} for full array: */
        ...
        if (verbose) 
          { fprintf(stderr, "ppv_chop(%d, " ppv_size_t_FMT ", %d)", i1, chunksz1, j1);
            ppv_print_descriptor(stderr, " = { ", &B, " }\n");
          } 
        /* Check validity of descriptor: */
        if (! ppv_descriptor_is_valid(&B, TRUE)) { bug0("ppv_chop returns an invalid desc"); }

        /* Check consistency of storage area: */
        if (B.el != A->el) { bug0("ppv_chop garbled storage area"); }
        /* Check whether the size of {1} is correct: */
        check_size(B.size, sz);
        /* Now check coincidence of the two arrays: */
        ppv_index_t ixA[N], ixB[N];
                ...
                for (ixB[1] = 0; ixB[1] < B.size[1]; ixB[1]++)
                  for (ixB[0] = 0; ixB[0] < B.size[0]; ixB[0]++)
                    { 
                      ...
                      ppv_pos_t posA = ppv_sample_pos(A, ixA);
                      if (posB != posA)
                        { bug2("ppv_chop pos error", " posB = ", ppv_pos_t_FMT, posB, " posA = ", ppv_pos_t_FMT, posA); } 
                    }
      }
  }
...
void test_replicate(ppv_array_t *A)
  {
    ...
    for (trial = 0; trial < ntrials; trial++)
      { /* Initialze {sz} for full array: */
        ...
        sz[i1] = rep1;
        if (verbose) 
          { fprintf(stderr, "ppv_crop(%d," ppv_size_t_FMT ",1)+ppv_replicate(%d," ppv_size_t_FMT ")", i1, skip1, i1, rep1);
            ppv_print_descriptor(stderr, " = { ", &B, " }\n");
          } 
        /* Check validity of descriptor: */
        ...
        sz[i2] = rep2;
        if (verbose) 
          { fprintf(stderr, "ppv_crop(%d," ppv_size_t_FMT ",1)+ppv_replicate(%d," ppv_size_t_FMT ")", i2, skip2, i2, rep2);
            ppv_print_descriptor(stderr, " = { ", &B, " }\n");
          } 
        /* Check validity of descriptor: */
        if (! ppv_descriptor_is_valid(&B, TRUE)) { bug0("ppv_replicate returns an invalid desc"); }
                ...
                for (ixB[1] = 0; ixB[1] < B.size[1]; ixB[1]++)
                  for (ixB[0] = 0; ixB[0] < B.size[0]; ixB[0]++)
                    { 
                      ...
                      ppv_pos_t posA = ppv_sample_pos(A, ixA);
                      if (posB != posA)
                        { bug2("ppv_replicate position error", " posB = ", ppv_pos_t_FMT, posB, " posA = ", ppv_pos_t_FMT, posA); } 
                    }
      }
  }
...
void check_size(ppv_size_t *sza, ppv_size_t *szb)
  {
    ppv_axis_t i;
    for (i = 0; i < N; i++)
      { if (sza[i] != szb[i]) { bug2("", "size", "[%d] = ", i, "", ppv_size_t_FMT, sza[0]); } }
  }
