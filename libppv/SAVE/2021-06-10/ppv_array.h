...
/* Last edited on 2021-06-10 15:56:37 by jstolfi */
...
ppv_sample_t ppv_get_sample ( ppv_array_t *A, ppv_index_t ix[] );
  /* Extracts the sample {A[ix[0],ix[1],.. ix[N-1]]}. */

void ppv_set_sample ( ppv_array_t *A, ppv_index_t ix[], ppv_sample_t qv );
  /* Stores value {qv} into the element {A[ix[0],ix[1],.. ix[N-1]]}. */
...
void ppv_slice ( ppv_array_t *A, ppv_axis_t ax, ppv_index_t ix );
  /* Modifies the descriptor {A} so that it describes the subset of samples
    where the index of axis {ax} are fixed to the value {ix}, 
    and then all following indices are rearranged so that the affected index
    (which now has size 1) ends up at the end. Thus, for example, after the call
    {ppv_slice(A,2,5)}, element {A[p,q,5,s,t,u]} of the original array 
    will be {A[p,q,s,t,u,0]}.  
    
    The axis {ax} must be in {0..N-1}, and the index {ix} must be valid for it.
