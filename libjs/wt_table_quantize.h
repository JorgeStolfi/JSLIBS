...
/* Last edited on 2023-11-23 11:30:34 by stolfi */
...
int32_t wt_table_quantize(int32_t n, double wd[], int32_t wi_min, int32_t wi_sum, int32_t wi[]);
  /* Stores into {wi[0..n-1]} a copy of the weights {wd[0..n-1]} scaled and rounded
    ...
    In any case, if {wi_min} is nonzero, the procedure will ensure that
    {|wi[k]|} will be at least {wi_min} whenever {wd[k]} is nonzero.
    Otherwise some non-zero weights {wd[k]} may be rounded to zero. In
    any case, if {wi[k]} is not zero, it will have the same sign as
    {wd[k]}.
    
    However, if all input weights {wd[0..n-1]} are zero, the output
    weights {wi[0..n-1]} will be all zero, regardless of the parameters.
    
    Returns the sum of the absolute values of {wi[0..n-1]}.
