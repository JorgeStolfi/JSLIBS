...
/* Last edited on 2023-03-18 11:07:18 by stolfi */ 
...
void generate_test_samples(int32_t *nsP, double **sP, double *zkerP);
...
void do_plot_test(char *prefix, int32_t order, interp_spline_kind_t kind)
  {
    ...
    int32_t nw = interp_spline_compute_num_samples(order, kind);
    if (nw == 0) 
   ...
   /* Choose the number of test samples {ns} and the samples {s[0..ns-1]}: */
    ...
    int32_t i[nw];       /* Tap positions in sample array. */
    ...
    int32_t HU = 64;    /* Half of subsamples per data sample. */
    int32_t NU = 2*HU;  /* Subssamples per data sample; must be even. */
    int32_t NP = NU*ns; /* Total subsamples. */
...
void generate_test_samples(int32_t *nsP, double **sP, double *zkerP)
  {
    ...
    int32_t ns = DX + W_ker + (deg_MAX+1)*(DX + W_out) + DX;
    ...
    int32_t ks = 0; /* Next sample to be defined is {s[ks]}. */
