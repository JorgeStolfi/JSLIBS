/* Last edited on 2023-02-18 18:38:46 by stolfi */
...
void plot_opf (int32_t N, r2_t st[], int32_t P[], char *prefix);
...
void draw_opf_edges(epswr_figure_t *eps, int32_t N, r2_t st[], int32_t P[]);
void draw_sites(epswr_figure_t *eps, int32_t N, r2_t st[], int32_t P[]);
...
int32_t main(int32_t argc, char **argv)
  { int32_t N = atoi(argv[1]);
    ...
    auto double Dist(int32_t i, int32_t j);         /* Euclidean distance between sites. */
    auto double Fmax(double Ca, double Cp); /* Returns {ca+Cp}. */
    auto double Fsum(double Ca, double Cp); /* Returns {ca+Cp}. */
    
    char *prefix = NULL;
    asprintf(&prefix, "opf_%04d_%s_%s", N, distr_name, pcost_name);
    ...
    int32_t *R = notnull(malloc(N*sizeof(int32_t)), "no mem"); /* Root map. */
    double *C = notnull(malloc(N*sizeof(double)), "no mem"); /* Cost map. */
    
    /* Pick three sites as roots: */
    { int32_t i; for (i = 0; i < N; i++) { C[i] = +INF; } }
    if (N >= 1) { C[0] = 0.0; }
    if (N >= 2) { C[1] = 0.0; }
    if (N >= 3) { C[2] = 0.0; }
    ...
    double Dist(int32_t i, int32_t j)
