...
/* Last edited on 2011-12-22 01:13:52 by stolfi */ 
...
typedef struct desc_t
  { ix_dim_t d;
    ...
    ix_pos_t b;
    ix_step_t s[MAXDIM];
    int nel;     /* Number of elements allocated for  {el}. */
...
desc_t *make_packed_desc(ix_dim_t d, ix_size_t sz[], ix_order_t ixor);
  /* Creates a descriptor with axis count {d} and
    size vector {sz[0..d-1]}, with steps consistent
    with a packed element arrangement with index
    order {L}, and no virtual replication. */

void do_test_suite(int nt);
...
void do_test_suite(int nt)
{
  ...
  for (it = 0; it < nt; it++)
    { 
      ix_dim_t d = (it % MAXDIM);
      test_size_ops(d);

      /* An array for testing: */
      sz[0] = abrandom(0, 2) + abrandom(0, 2); 
      sz[1] = abrandom(0, 5) + abrandom(0, 5); 
      sz[2] = 1; 
      sz[3] = abrandom(0, 9) + abrandom(0, 9); 
      sz[4] = abrandom(0, 7) + abrandom(0, 7);
      sz[5] = abrandom(0, 3) + abrandom(0, 3);
      assert(MAXDIM == 6);
...
desc_t *make_desc(ix_dim_t d, ix_size_t sz[])
{
  ...
  ix_axis_t iz = abrandom(0,2*d+1);     /* The index that will get zero step. */
  A->d = d;
  int i;
  int64_t npos = 1;
  for (i = 0; i < d; i++)
  ...
  if (npos == 0)
    { for (i = 0; i < d; i++) { A->s[i] = 0; } 
      A->b = 0;
    }
  else
    { /* A random base: */
      A->b = 1000*abrandom(1,9);
    }
  if (! ix_parms_are_valid(A->d, A->z, A->b, A->s, TRUE))
...
void test_size_ops(ix_dim_t d)
{
  ...
  auto ix_size_t rnd_sz(int a, int b, int c, int i); 
    /* A funct of {a,b,c}, and {i}. */

  ix_size_t rnd_sz(int a, int b, int c, int i) 
    { return a + ((b*i*i) % c); }
  ...
  for (i = 0; i < MAXDIM; i++) { sza[i] = szc[i] = rnd_sz(17,  8, 13, i); }
  for (i = 0; i < MAXDIM; i++) { szb[i] = rnd_sz(19, 10, 17, i); }
  ...
  for (i = 0; i < MAXDIM; i++) { sza[i] = rnd_sz(17,  8, 13, i); }
...
void test_addressing(desc_t *A)
{
  ...
  int64_t npos = 1;
  { int i; for (i = 0; i < A->d; i++) { npos *= A->z[i]; } }
          ...
          for (ixA[1] = 0; ixA[1] < sz[1]; ixA[1]++)
            for (ixA[0] = 0; ixA[0] < sz[0]; ixA[0]++)
              { 
                ...
                for (i = 0; i < A->d; i++) { pos_exp += A->s[i]*ixA[i]; }
...
void test_enum(desc_t *A)
{
  ...
  ix_pos_count_t npos = ix_num_tuples(d, sz);
...
void test_packed(desc_t *A)
{
  ...
  desc_t *F = make_packed_desc(d, A->z, ix_order_F);
  desc_t *L = make_packed_desc(d, A->z, ix_order_L);
  bool_t verbose = FALSE;
  if (verbose) 
    { print_desc(stderr, "A = { ", A, " }\n");
  ...
  /* Allocate a vector to store all positions of {B}: */
  ix_pos_count_t npos = ix_num_tuples(d, sz);
  int64_t kpos;     /* Current index into {posA}. */
    ...
    for (ix[4] = 0; ix[4] < sz[4]; ix[4]++)
      for (ix[3] = 0; ix[3] < sz[3]; ix[3]++)
        for (ix[2] = 0; ix[2] < sz[2]; ix[2]++)
          for (ix[1] = 0; ix[1] < sz[1]; ix[1]++)
            for (ix[0] = 0; ix[0] < sz[0]; ix[0]++)
              { ix_pos_t posF1 = ix_packed_position(d, ix, sz, ix_order_F);
                ...
                assert(posF1 == kpos);
                assert(posF2 == posF1);
                ix_packed_indices(d, kpos, sz, ix_order_F, jx);
                for (i = 0; i < d; i++) { assert(ix[i] == jx[i]); }
                kpos++;
              }
  assert(kpos == npos);
    ...
    for (ix[1] = 0; ix[1] < sz[1]; ix[1]++)
      for (ix[2] = 0; ix[2] < sz[2]; ix[2]++)
        for (ix[3] = 0; ix[3] < sz[3]; ix[3]++)
          for (ix[4] = 0; ix[4] < sz[4]; ix[4]++)
            for (ix[5] = 0; ix[5] < sz[5]; ix[5]++)
              { ix_pos_t posL1 = ix_packed_position(d, ix, sz, ix_order_L);
                ix_pos_t posL2 = ix_position(d, ix, L->b, L->s);
                assert(posL1 == kpos);
                assert(posL2 == posL1);
                ix_packed_indices(d, kpos, sz, ix_order_L, jx);
                for (i = 0; i < d; i++) { assert(ix[i] == jx[i]); }
                kpos++;
              }
  assert(kpos == npos);
...
void test_crop(desc_t *A)
{
  ...
  for (trial = 0; trial < ntrials; trial++)
    { /* Initialze {sh} and {sz} for full array: */
      { int i; for (i = 0; i < MAXDIM; i++) { sh[i] = 0; } }
      extend_size_vector(A->d, A->z, MAXDIM, sz);
...
void test_subsample(desc_t *A) 
{
  ...
  for (trial = 0; trial < ntrials; trial++)
    { /* Initialze {st} and {sz} for full array: */
      { int i; for (i = 0; i < MAXDIM; i++) { st[i] = 1; } }
      extend_size_vector(A->d, A->z, MAXDIM, sz);
      /* Start with the standard array: */
      desc_t B = (*A);
            ...
            for (ixB[2] = 0; ixB[2] < sz[2]; ixB[2]++)
              for (ixB[1] = 0; ixB[1] < sz[1]; ixB[1]++)
                for (ixB[0] = 0; ixB[0] < sz[0]; ixB[0]++)
                  { 
                    ix_axis_t ax;
                    for (ax = 0; ax < A->d; ax++) { ixA[ax] = ixB[ax]*st[ax]; }
                    ix_pos_t posB = ix_position(B.d, ixB, B.b, B.s);
                    ix_pos_t posA = ix_position(A->d, ixA, A->b, A->s);
                    if (posB != posA)
...
void test_slice(desc_t *A)
{
  ...
  for (trial = 0; trial < ntrials; trial++)
    { /* Start current slice with the given array: */
      ...
      { int i; for (i = 0; i < MAXDIM; i++) { axO[i] = i; ixO[i] = -1; } }
      
      int pass;
      for (pass = 0; pass < 2; pass++)
        { 
          /* At this point, axis {k} of current slice is axis {axO[k]} of original, or trivial if {axO[k]==A->d}. */
          /* At this point, if {ixO[k]} is non-negative, then axis {k} of original was set to {ixO[k]}. */
          
          /* Choose some axes to fix, and their index values: */
          int nx1 = 0;            /* Number of axes to fix. */
          ix_axis_t ax1[MAXDIM];  /* Which axes to fix. */
          ix_index_t ix1[MAXDIM]; /* Index values of those axes. */
          ix_axis_t *axp = (((toss/9) % 3) == 0 ? NULL : ax1);
          int nxp = ((toss/11) % (B.d + 1)); /* Number of axes to fix if {axp = NULL}. */
          { ix_axis_t i;          /* Axis of current slice. */
          ...
          if (verbose) 
            { fprintf(stderr, "pass %d slice(%d, %s{", pass, nx1, (axp == NULL ? "[NULL]" : ""));
              { int k; for (k = 0; k < nx1; k++) { fprintf(stderr, (" [%d]=%" int64_d_fmt ""), ax1[k], ix1[k]); } }
              fprintf(stderr, " })");
              print_desc(stderr, "  B = { ", &B, " }\n");
          ...
          for (ixB[3] = 0; ixB[3] < sz[3]; ixB[3]++)
            for (ixB[2] = 0; ixB[2] < sz[2]; ixB[2]++)
              for (ixB[1] = 0; ixB[1] < sz[1]; ixB[1]++)
                for (ixB[0] = 0; ixB[0] < sz[0]; ixB[0]++)
                  { 
                    { ix_axis_t i; for (i = 0; i < A->d; i++) { if (ixO[i] >= 0) { ixA[i] = ixO[i]; } } }
                    { ix_axis_t j;
                      for (j = 0; j < B.d; j++) 
                        { ix_index_t i = axO[j];
...
void test_swap_indices(desc_t *A)
{
  ...
  for (trial = 0; trial < ntrials; trial++)
    { /* Initialze {tr} for full array: */
      { int i; for (i = 0; i < MAXDIM; i++) { tr[i] = i; } }
      /* Start with the standard array: */
      desc_t B = (*A);
      ...
      for (pass = 0; pass < 2; pass++)
        { /* Choose two axes {ax1,bx1} and the axis count {nx1}: */
          ix_axis_t ax1 = (trial % A->d);
          ix_axis_t bx1 = ((trial/2) % A->d);
          int dx1 = (ax1 == bx1 ? A->d - ax1 : (ax1 > bx1 ? ax1 - bx1 : bx1 - ax1));
          assert(dx1 > 0);
          int nx1 = ((trial/3) % dx1);
          assert(ax1 + nx1 <= A->d);
          assert(bx1 + nx1 <= A->d);
            ...
            for (ixA[2] = 0; ixA[2] < sz[2]; ixA[2]++)
              for (ixA[1] = 0; ixA[1] < sz[1]; ixA[1]++)
                for (ixA[0] = 0; ixA[0] < sz[0]; ixA[0]++)
                  { 
                    ix_axis_t ax;
                    for (ax = 0; ax < A->d; ax++) { ixB[ax] = ixA[tr[ax]]; }
                    ix_pos_t posB = ix_position(B.d, ixB, B.b, B.s);
                    ix_pos_t posA = ix_position(A->d, ixA, A->b, A->s);
                    if (posB != posA)
...
void test_flip(desc_t *A)
{
  ...
  for (trial = 0; trial < ntrials; trial++)
    { /* Initialze {fp} for unflipped array: */
      { int i; for (i = 0; i < MAXDIM; i++) { fp[i] = FALSE; } }
      /* Start with the standard array: */
      desc_t B = (*A);
...
void test_diagonal(desc_t *A)
{
  ...
  for (trial = 0; trial < ntrials; trial++)
    { /* Initialze {sz} for full array: */
      ...
      /* Choose any two axes: */
      ix_axis_t ax1 = (trial % A->d), bx1 = (trial/A->d % A->d);
      if (bx1 == ax1) { bx1 = (ax1 + 1) % A->d; }
      /* Make it square: */
...
{
  int i;
  for (i = 0; i < dB; i++) { szB[i] = (i < dA ? szA[i] : 1); }
}
