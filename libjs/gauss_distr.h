...
/* Last edited on 2019-12-07 14:44:59 by jstolfi */
...
double gauss_distr_CDF(double x, double avg, double dev);
  /* The cumulative distribution function of a Gaussian deviate 
    ...
    The sign of {dev} is ignored. If {dev} is {±INF},
    returns 0 {x < +INF} and {NAN} for {x=+INF}.  If {dev} is zero,
...
double gauss_distr_integral(double x0, double x1, double avg, double dev);
  /* Returns the integral from {x0} to {x1} of the normalized Gaussian 
    ...
    that a Gaussian deviate with those parameters will be between {x0} and {x1}.
     
    The sign of {dev} is ignored. If {dev} is zero, returns {+1} if {x0<avg<x1},
    {-1} if {x1<avg<x0}, 0 if {x0,x1} are on the same side of avg, and {NAN}
    if either of them is equal to {avg}.  If {dev} is nonzero and {x0=x1}, 
    returns 0. */
