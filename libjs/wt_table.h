...
/* Last edited on 2022-10-30 19:36:16 by stolfi */
...
void wt_table_fill_binomial(int32_t n, double wt[], bool_t norm);
  /* If {norm} is false, sets {wt[i]} to {choose(i,n-1)} for {i} in {0..n-1}. 
    ...
    sum will be 1.  In that case, the table will then be a partition of unity
    if used with stride 1 or (if n >=2) with stride 2. The unit sum and partition
    of unit properties will be exact for {n} up to {40} at least. */
...
void wt_table_fill_triangular(int32_t n, double wt[], bool_t norm);
  /* First, builds a triangular weight table, {wt[i] = 1 - |i-c|/h}
    ...
    If {norm} is true, the table is normalized so that the sum of all entries is 1.
    In that case, if {n} is odd, {2*r+1}, the table will be a partition of unity when
...
double_vec_t wt_table_make_gaussian(double sigma, double maxLoss, bool_t norm);
  /* Builds a weight table {wt} derived from a Gaussian distribution {G(z)}
    ...
    as in {wt_table_fill_gaussian(sigma,n,wt,norm)}. */
...
void wt_table_shifted_sum(int32_t n, double wt[], int32_t stride, double ws[]);
  /* Returns in {ws[0..n-1]} the result of adding all copies of {wf} shifted by integer multiples of {stride}.
    Namely, sets {ws[k]} to the sum of {wf[k+i*stride]} for all
    integer {i}, assuming that non-existent element of {wf} are zero.
    Requires {stride >= 1}. */
...
bool_t wt_table_check_normalization(int32_t n, double wt[], double tol, bool_t die);
  /* Checks whether {wtb.e[0..wtb.ne-1]} add to 1, with tolerance {tol}. If not, either dies
    with error (if {die} is {TRUE}) or returns {FALSE} silently (if {die} is
    {FALSE}). */
...
bool_t wt_table_check_partition_of_unity(int32_t n, double wt[], int32_t stride, double tol, bool_t die);
  /* Checks whether the sum of copies of {wt} shifted by all integer
    ...
    all elements equal to 1, within the tolerance {tol}. If  not, either
    dies with error (if {die} is {TRUE}) or returns {FALSE} silently (if {die} is {FALSE}). */
