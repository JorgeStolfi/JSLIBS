...
/* Last edited on 2017-10-17 22:58:51 by jstolfi */
...
void neuromat_eeg_header_free(neuromat_eeg_header_t *h)
  {
    if (h->chnames != NULL)
      { demand(h->nc > 0, "cannot have {chnames} without {nc}");
        int ic;
        for (ic = 0; ic < h->nc; ic++) { free(h->chnames[ic]); }
        free(h->chnames);
      }
...
void neuromat_eeg_header_write(FILE *wr, neuromat_eeg_header_t *h)
  {
    ...
    if (h->chnames != NULL) 
      { demand(h->nc > 0, "cannot have {channels} without {nc}");
        fprintf(wr, "channels =");
        int ic;
        for (ic = 0; ic < h->nc; ic++) { fprintf(wr, " %s", h->chnames[ic]); }
        fprintf(wr, "\n");
      }   
...
void neuromat_eeg_header_merge(neuromat_eeg_header_t *dst, neuromat_eeg_header_t *src)
  {
    ...
    neuromat_eeg_header_merge_int(&(dst->tkeep), src->tkeep, "tkeep");     
...
int neuromat_eeg_header_append_electrode_channel(neuromat_eeg_header_t *h, char *name)
  {
    demand((h->nc != INT_MIN) && (h->chnames != NULL) && (h->ne != INT_MIN), "uninitialized header");
    int ie_new = h->ne;
    h->ne++;
    h->nc++;
    h->chnames = notnull(realloc(h->chnames, h->nc*sizeof(char*)), "no mem");
    for (int ic = h->nc-1; ic > ie_new; ic--)
