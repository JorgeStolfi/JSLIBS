...
/* Last edited on 2024-05-02 10:09:58 by stolfi */
  ...
  )
  {
    ...
    for (int32_t ki = 0; ki < NI; ki++)
      { char *zTag = NULL;
        ...
        csimg[ki] = multifok_stack_read_color_image(inPrefix, zTag); 
        if (ki == 0)
          { float_image_get_size(csimg[ki], &NC, &NX, &NY); }
        else
          { float_image_check_size(csimg[ki], NC, NX, NY); }

        shimg[ki] = multifok_stack_read_sharpness_image(inPrefix, zTag); 
...
int32_t multifok_stack_choose_object_count(interval_t dom[], double rMin, double rMax, double minSep);
  /* Chooses the ideal number of objects (disks or balls) that {multifok_stack_throw_busy} 
    should try to put in a scene.  The parameter {minSep} has the meaning described 
    under {multifok_stack_throw_busy}. */

void multifok_stack_throw_colors(frgb_t *tone, frgb_t *bg, frgb_t *fg);
  /* Generates two random contrasting colors, with general hue depending on {warm}. */

typedef struct multifok_stack_tree_t
  { multifok_stack_object_t *obj; 
    int8_t axis;                            /* Axis perpendicular to split plane, 0 or 1. */
    interval_t bbox[3];                     /* Bounding box of all objects. */
    struct multifok_stack_tree_t *child[2]; /* Children nodes, or NULL. */
  } multifok_stack_tree_t;
...
void multifok_stack_sort_objects(int32_t NO, multifok_stack_object_t objs[], int8_t axis);
  /* Rearranges {objs[0..NO-1]} in increasing order of coordinate {axis} (0 or 1). */

void multifok_stack_check_objs(int32_t NO, multifok_stack_object_t objs[], int8_t axis);
...
multifok_stack_tree_t *multifok_stack_build_tree(int32_t NO, multifok_stack_object_t objs[]);
  /* Builds an acceleration tree for the objects {objs[0..NO-1]}.  Will rearrange the
    objects in the process. */
    
void multifok_stack_ray_trace_tree
  ( multifok_stack_tree_t *tr, 
    r3_t *p, 
    r3_t *d, 
    double zMin,
    bool_t debug, 
    int32_t level,
    multifok_stack_object_t **htob_P, 
    r3_t *htpt_P
  );
...
bool_t multifok_stack_ray_trace_object(multifok_stack_object_t *obj, r3_t *p, r3_t *d, double zMin, bool_t debug, r3_t *htpt_P);
...
void multifok_stack_ray_trace_floor(interval_t dom[], bool_t flatFloor, r3_t *p, r3_t *d, bool_t debug, r3_t *htpt_P);   
  /* Traces one ray {R} that goes through the point {p} with the direction parallel to {d}, assumed to be 
    ...
    described under {multifok_stack_t}. */
    
void multifok_stack_get_ray_bbox(r3_t *p, r3_t *d, double zMin, interval_t bbox[]);
  /* Sets {bbox[0..2]} to the bounding box of the part of the ray between {Z=zMin} and {Z=ZMAX}. */
  
/* IMPLEMENTATIONS */

multifok_stack_t *multifok_stack_new(interval_t dom[], bool_t flatFloor, bool_t verbose)
  {
    demand((dom[2].end[0] >= 0.0) && (dom[2].end[1] <= ZMAX), "bad {dom[2]}");
     
    /* Allocate record: */
    multifok_stack_t *scene = (multifok_stack_t *)notnull(malloc(sizeof(multifok_stack_t)), "no mem");
...
void multifok_stack_throw_objects
  ( multifok_stack_t *scene,
    double rMin, 
    double rMax,
    double minSep, 
    bool_t verbose
  )
  {
    ...
    int32_t NO_max = multifok_stack_choose_object_count(dom, rMin, rMax, minSep);
    if (verbose) { fprintf(stderr, "trying to generate %d objects\n", NO_max); }
    multifok_stack_object_t *objs = (multifok_stack_object_t*)notnull(malloc(NO_max*sizeof(multifok_stack_object_t)), "no mem");
    ...
    for (int32_t kt = 0; (kt < NT) && (NO < NO_max); kt++)
      { /* Generate a random object {obj}: */
        multifok_stack_object_t obj = multifok_stack_object_throw(NO, dom, minSep, rMin, rMax);
        bool_t ok = TRUE; /* False if object overlaps the previous ones in {XY}. */
        if (minSep >= 0)
          { /* Check for {XY} overlaps: */
            for (int32_t jd = 0; (jd < NO) && ok; jd++)
              { multifok_stack_object_t *objj = &(objs[jd]);
    ...
    if (NO < NO_max)
      { if (verbose) { fprintf(stderr, "generate only %d objects\n", NO); }
        /* Trim array: */
        objs = (multifok_stack_object_t*)notnull(realloc(objs, NO*sizeof(multifok_stack_object_t)), "no mem");
      }
    ...
    scene->tree = multifok_stack_build_tree(NO, objs);
    /* Check that the objects are still OK: */
    multifok_stack_check_objs(NO, objs, -1);
...
multifok_stack_object_t multifok_stack_object_throw(int32_t ID, interval_t dom[], double minSep, double rMin, double rMax)
  {
    ...
    multifok_stack_throw_colors((obj.flat ? &tone_disk : &tone_ball), &(obj.bg), &(obj.fg));
...
multifok_stack_tree_t *multifok_stack_build_tree(int32_t NO, multifok_stack_object_t objs[])
  {
    if (NO == 0)
      ...
      { multifok_stack_tree_t *tr = (multifok_stack_tree_t *)notnull(malloc(sizeof(multifok_stack_tree_t)), "no mem");
        tr->child[0] = NULL;
        tr->child[1] = NULL;
        ...
        if (NO == 1)
          ...
          { /* We must choose {axis}, sort the objects, build {child[0],child[1]}. */
            ...
            multifok_stack_sort_objects(NO, objs, tr->axis);
          }
        ...
        for (int32_t ic = 0; ic < 2; ic++)
          { /* Define the objects {objs[ko_min..ko_max]} that go into {child[ic]}: */
            ...
            if (NO_ch == 0)
              ...
              { /* Build {child[ic]}: */
                multifok_stack_tree_t *ch = multifok_stack_build_tree(NO_ch, &(objs[ko_min]));
                assert(ch != NULL);
...
void multifok_stack_sort_objects(int32_t NO, multifok_stack_object_t objs[], int8_t axis)  
  { auto int32_t objcomp(const void*a, const void*b);
    ...
    /* multifok_stack_check_objs(NO, objs, -1); */
    if (NO > 1) { qsort((void*)objs, NO, sizeof(multifok_stack_object_t), objcomp); }
    /* multifok_stack_check_objs(NO, objs, axis); */

    return;
            
    int32_t objcomp(const void*a, const void*b)
      { multifok_stack_object_t *oba = (multifok_stack_object_t *)a;
        multifok_stack_object_t *obb = (multifok_stack_object_t *)b;
...
void multifok_stack_ray_trace
  ...
  )
  {
    ...
    multifok_stack_tree_t *tr = scene->tree;
    ...
    multifok_stack_object_t *obj_htob; /* Highest object that was hit, or {NULL}. */
    r3_t obj_htpt; /* Hit point on {obj_htob}, or {(NAN,NAN,NAN)} */
    double zMin = 0.0 - FUDGE; /* Assume that the ray does not go below this {Z}. */
    int32_t level = 0;
    multifok_stack_ray_trace_tree(tr, p, d, zMin, debug, level, &obj_htob, &obj_htpt);
    if (verbose) 
      { if (obj_htob == NULL) 
    ...
    r3_t flo_htpt;
    multifok_stack_ray_trace_floor(scene->dom, scene->flatFloor, p, d, debug, &flo_htpt);
 
    /* Decide if ray hit floor or objects first. */
    multifok_stack_object_t *htob; /* Object that was hit, or {NULL}. */
...
void multifok_stack_ray_trace_tree
  ...
  { 
    ...
    multifok_stack_object_t *htob = NULL;
      ...
      { if (verbose) { fprintf(stderr, "    %*sray zMin = %12.8f tree zMax = %12.8f\n", 2*level, "", zMin, tr->bbox[2].end[1]); }
        if (zMin <= tr->bbox[2].end[1])
          { /* Get the ray's bounding box {ray_bbox[0..2]}: */
            ...
            multifok_stack_get_ray_bbox(p, d, zMin, ray_bbox);
            ...
            if (ok)
              { /* Bounding boxes intersect, we have a chance. */
                ...
                multifok_stack_object_t *obj = tr->obj;
                r3_t obj_htpt;
                bool_t obj_hit = multifok_stack_ray_trace_object(obj, p, d, zMin, debug, &obj_htpt);
                if (obj_hit)
                ...
                for (int32_t kc = 0; kc < 2; kc++) 
                  { /* Ray-trace {child[ic]} */
                    ...
                    multifok_stack_object_t *ch_htob;
                    r3_t ch_htpt;
                    multifok_stack_ray_trace_tree(tr->child[ic], p, d, zMin, debug, level+1, &ch_htob, &ch_htpt);
                    if (ch_htob != NULL)
                      { /* We got a hit: */
...
void multifok_stack_ray_trace_floor(interval_t dom[], bool_t flatFloor, r3_t *p, r3_t *d, bool_t debug, r3_t *back_htpt_P)
  {     
    bool_t verbose = debug;
...
frgb_t multifok_stack_compute_hit_color(multifok_stack_object_t *obj, r3_t *q, multifok_stack_pattern_t *pattern)
