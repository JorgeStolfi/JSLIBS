...
/* Last edited on 2023-01-30 15:22:25 by stolfi */
...
/* QUADRATIC TERMS FOR SHARPNESS ESTIMATION
  ...
  either 1 or 0. Therefore each term is a sum of products of paits of
  basis coefficients {b[jb1]*b[jb2]}, where {b[0..NB-1]} is the coeff
  vector {b = s*B}, and {0 <= jb1 <= jb2 < NB}.
  
  Moreover we assums that no two matrices {Q[t']}, {Q[t'']} have "1" in
...
typedef struct multifok_term_prod_t
  { int32_t jb1; /* Index of first basis elem coeff. */
    ...
    char *name;  /* Name of term, e.g. "DX*DX" of "Smm*Spp". */
  } multifok_term_prod_t;
  /* Record describing a product of two coeffs {coeff[jb1]*coeff[jb2]}
    in some local lino basis, that contributes to one indpendent term of
    a quadratic local sharpness estimator. 
    
    The product must be in canonical form, meaning {jb1<=jb2}. The name shoud be "{el1}*{el2}"
    where {el1} and {el2} are the names of the correspoing basis elements.
    
    As a special case, {jb1} and {jb2} may be {-1}, representing the empty product 
    whose value is constant 1.  The {name} then should be "1". */
...
void multifok_term_values_from_basis_coeffs
  ...
  /* Assumes that {coeff[0..NB-1]} are the coefficients of the window
    ...
    Namely, for each entry {prix[k] = (jb1,jb2,kt,name)}, computes the
...
void multifok_term_indices_from_names
  ...
  /* The parameter {termName[0..NT-1]} must be a list of formulas of
    ...
    Each product should appear only once, and each term must have at least one product. 
    Thus {NP} is at most {NB*(NP+1)/2}, and {NT} is at most equal to {NP}.
    
    The product count {NP} and the table {prix} are returned in {*NP_P}
    and {*prix_P}. The {name} fields in the {prix} table will be newly
    allocated strings. */
...
void multifok_term_read_index_table
  ...
  /* Reads from {rd} a table describing how a certain number {NT} of terms of degree 2 are to be 
    composed from {NP} products of pairs of {NB} basis coefficients.
  
    The file has one line for each coeff product to be evaluated. Each
    line has the format "{ip} {jb1} {jb2} {kt} {name}" where {ip} is the product
    index in {0..NP-1}, {jb1} and {jb2} are the indices of the two basis
    coeffs, each in {0..NB-1}, {kt} is the index of the term to which
    this product is part, and {name} is the textual product
    "{el1}*{el2}", e.g. "Xom*Xpp"; where {el1} must be {belName[jb1]} and 
    {el2} must be {belName[jb2]}.
    
    The products must be all distinct, and, ccounting for the
    commutativity of "*", every entry must have {jb1<=jb2}; so the
    maximum value of {NP} is {NB*(NB+1)/2}. The quadruples {(jb1, jb2,
    kt, name)} are saved in a table {prix[0..NP-1]}.
...
void multifok_term_write_names(FILE *wr, int32_t NT, char *termName[]);
  /* Writes to {wr} the term names {termName[0..NT-1]}, one per line. */
