...
/* Last edited on 2025-02-06 21:22:45 by stolfi */
...
frgb_t multifok_scene_raytrace_compute_hit_color
  ...
  { 
    bool_t debug = FALSE;
    demand((cIso >= 0.0) && (cIso <= 1.0), "invalid isotropic light color");
    if (obj == NULL)
      ...
      { interval_t *bbox = obj->bbox; 
        ...
        double r = pattern(&u);
        /* Get the lambedo {sLam} as an {r}-mix of the object's {fg} and {bg}: */
        frgb_t sLam = frgb_mix((1-r), &(obj->bg), r, &(obj->fg));
        if ((vLit != NULL) || (cIso < 1.0))
          { /* Apply shading: */
            ...
            double dot = r3_dot(&onorm, vLit);
            double diffuse  = (dot <= 0 ? 0.0 : (1-cIso)*dot);
            double shade = cIso + diffuse;
            for (uint32_t j = 0;  j < 3; j++){ sLam.c[j] = (float)(shade*sLam.c[j]); }
          }
        return sLam;
      }
  }
