...
/* Last edited on 2008-01-29 20:33:08 by hcgl */
...
msm_ps_tools_t *msm_ps_tools_new
  ...
  /* Creates a {msm_ps_tools_t} for an Encapsulated Postscript
    ...
    The {fontSize} will be used for labels and titles of 
...
void msm_ps_tools_draw_graphs
  ...
  /* Draws {nc} graphs on {mps}, each with a different color. 
    ...
    If {circ} is true the graphs are assumed to be periodic, and an
    extra segment is drawn at each end of the plot to show that. In
    that case the vector {x} must have {nd+1} elements, and the period
    is assumed to be {x[nd] - x[0]}.
