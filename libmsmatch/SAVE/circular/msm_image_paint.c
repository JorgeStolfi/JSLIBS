...
/* Last edited on 2008-04-19 12:07:02 by stolfi */
  ...
  )
  {
    ...
    for (ic = 0; ic < cdv->ne; ic++)
      { /* Get the candidate {cd} with index {ic}: */
        ...
        demand(cd->seq[0].npos == xp->npos, "inconsistent xp->npos");
        demand(cd->seq[1].npos == yp->npos, "inconsistent yp->npos");
        /* Paint it: */
