...
/* Last edited on 2008-04-19 07:31:05 by stolfi */
...
typedef struct msm_cand_t
  ...
  /* A candidate pairing between two abstract sequences 
    ...
    range {0..seq[j].npos-1}. Moreover, if {seq[j].circ} is
    ...
    {c[j]} that exceed {seq[j].npos}. Those indices should be taken
    modulo {seq[j].npos} when indexing into sequence {seq[j]}.
    ...
    {per.c[j] % seq[j].npos == 0} and {per.c[j] > 0} (i.e., it must
...
msm_cand_t msm_cand_throw
  ...
  /* Generates a random candidate between two abstract sequences
    ...
    {msm_pairing_throw(xp->npos,xp->circ, yp->npos,yp->circ, 
    len,circp,atomic,diagonal,skipProb)}. */
