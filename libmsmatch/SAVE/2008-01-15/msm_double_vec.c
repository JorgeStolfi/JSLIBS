...
void msm_double_vec_smooth(double_vec_t *smp)
  { int ns = smp->nel;
    ...
    double s = 0;
    for (i = 0; i < ns; i++) { s += smp->el[i]; smp->el[i] = s; }
    /* Add a linear ramp to make the sequence periodic: */
    double ds = s/ns;
    for (i = 0; i < ns; i++) { smp->el[i] -= ds*i; }
  }
...
void msm_double_vec_normalize(double_vec_t *smp)
  { int ns = smp->nel;
    ...
    /* Compute average {avg}: */
    double sum = 0;
    for (i = 0; i < ns; i++) { sum += smp->el[i]; }
    double avg = sum/ns;
    /* Shift to zero mean: */
    for (i = 0; i < ns; i++) { smp->el[i] -= avg; }
    /* Compute deviation {dev}: */
    double sum2 = 0;
    for (i = 0; i < ns; i++) { double si = smp->el[i]; sum2 += si*si; }
    double dev = sqrt(sum2/ns);
    if (dev > 0)
      { /* Scale to unit variance: */
        for (i = 0; i < ns; i++) { smp->el[i] /= dev; }
      }
  }

double_vec_t msm_double_vec_throw_normal(int ns)  
  { double_vec_t smp = double_vec_new(ns);
    rn_throw_normal(smp.nel, smp.el);
    return smp;
  } 
...
void msm_double_vec_mutate
  ...
  { int no = smpo->nel; /* Number of samples in original sequence. */
    ...
    double sNext = (no == 0 ? 0.0 : smpo->el[imod(io,no)]);  /* Next sample from {smpo}. */
    double sPrev = (circ ? smpo->el[no-1] : sNext); /* Previous sample from {smpo}. */
    while (TRUE)
      { sign_t op = msm_double_vec_mutate_step(delProb);
        ...
        if (op >= 0)
          { /* Insert a new sample or copy the next sample. */
            ...
            smpd->el[nd] = sPlus;
            if (op == 0)
              { /* Record the inheritance of {smpd.el[nd]} from {smpo.el[io]}: */
                msm_rung_vec_expand(gv, ng); 
                gv->el[ng] = (msm_rung_t){{ io, nd }};
                ng++;
              }
        ...
        if (op <= 0) 
          { /* Skip the sample {smp[io]}: */ 
            ...
            /* Update {sNext}: */
            sNext = ((! circ) && (io >= no) ? sPrev : smpo->el[imod(io,no)]);
          }
      }
    /* Trim and return: */
