/* See msm_ps_tools.h */
/* Last edited on 2008-01-12 00:57:56 by stolfi */
...
msm_ps_tools_t *msm_ps_tools_new
  ...
  { /* Join {name} and {tag} into a file name prefix: */
    ...
    PSStream *ps = pswr_new_stream(TRUE, prefix, wr, NULL, hPageSize*mm, vPageSize*mm);
...
msm_ps_tools_t *msm_ps_tools_new_graph
  ...
  )
  {
    ...
    /* Create plot object: */
    msm_ps_tools_t *mps = msm_ps_tools_new(wr, name, tag, hTotSize, vTotSize, mrg);
...
void msm_ps_tools_draw_graphs
  ...
  )
  {
    ...
    double yScaleWd = 12.0; /* Width of Y scale and tics. */
    double xScaleHt =  5.0; /* Height of X scale and tics. */
    ...
    double fontSz = 12.0; /* In Postscript points. */
    double ticSz = 1.0;  /* In mm. */
    pswr_set_label_font(ps, font, fontSz);
      ...
      { /* Plot channel {c}. */
            ...
            if ((ip >= 0) && (ip < nd))
              ...
              { assert(circ);
                if (ip == -1)
                  ...
                  { assert(FALSE); }
                /* Interpolate between data points {ix-1} and {ix}: */
                double x0, y0;
                get_data_point(ip, &x0, &y0);
                assert(x0 <= (*xp));
                double x1, y1;
                get_data_point(ip+1, &x1, &y1);
                assert((*xp) <= x1);  
        ...
        while (i <= iFin)
          { /* Plot step from sample {i-1} to sample {i}: */
            ...
            /* ??? */ demand(x1 - x0 >= 1, "abscissas are not 1-increasing");
            double h1, v1;
