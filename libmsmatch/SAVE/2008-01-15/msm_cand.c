/* See msm_cand.h */
/* Last edited on 2008-01-12 12:12:18 by stolfi */
...
bool_t msm_cand_is_valid(msm_cand_t *cd, bool_t die)
  {
    ...
    if (ng != 0) 
      { /* Get datum counts of the two sequences: */
  ...
  { int j;
    for (j = 0; j < 2; j++)
      { msm_seq_desc_t *saj = &(ca->seq[j]);
        ...
        demand(saj->nbas == sbj->nbas, "inconsistent seq lengths"); 
      }
    int nx = ca->seq[0].nbas, ny = ca->seq[1].nbas;
...
double msm_cand_compute_score
  ...
  { int ng = msm_pairing_num_rungs(pr);
    ...
    msm_rung_t g = (circp ?  msm_pairing_get_rung(pr, ng-1) : msm_rung_none);
    int t; 
...
msm_cand_t msm_cand_map_to_finer(msm_cand_t *cd, int nxnew, int nynew, int nwtb)
  { 
    ...
    int nold[2]; /* Counts of bases in old sequences. */
    msm_cand_t cdnew;
    cdnew.score = cd->score; 
    int j;
    for (j = 0; j <= 1; j++)
      { nold[j] = cd->seq[j].nbas;
        cdnew.seq[j] = cd->seq[j];
        cdnew.seq[j].nbas = (j == 0 ? nxnew : nynew);
        cdnew.seq[j].level --;
      }
    cdnew.pr = msm_pairing_map_to_finer(cd->pr, nold[0], nold[1], nxnew, nynew, nwtb);
