/* See msm_cand_vec.h */
/* Last edited on 2008-01-12 12:12:33 by stolfi */
...
msm_cand_vec_t msm_cand_vec_get_best_perfect
  ...
  {
    int nx = xp->nbas; bool_t circx = xp->circ;
    int ny = yp->nbas; bool_t circy = yp->circ;
    
    int64_t minRungs = minBases; 
      /* Min number of rungs in a candidate, including the circular one. */
    ...
    auto void scan_alignment(int ix, int iy, int64_t ng, bool_t circp);
      /* Scans the perfect pairing {p} that starts with rung {(ix,iy)}
        ...
        that has at least {minBases} and at most {min(nx,ny)} rungs. A
    ...
    void scan_alignment(int ix, int iy, int64_t ng, bool_t circp)
      {
        ...
        void initialize_score_tables(void)
          { ksize = 2*maxRungs - minRungs + 1;
            ...
            rsc.el[0] = step_score(xp, yp, msm_rung_none, g);
            psc.el[0] = rsc.el[0];
            kmin = 0; kmax = 0; 
          }
          ...
          { 
            ...
            do
              { int64_t k = kmax + 1;
                ...
                int kk = imod(k,ksize);
                psc.el[kk] = psc.el[imod(kmax,ksize)] + ssck;
                g = h;
                kmax++;
        ...
        double join_score(int64_t k)
          { demand(k >= 0, "invalid prefix: k < 0");
            ...
            update_score_tables(k);
            affirm((kmin <= k) && (k <= kmax), "out of table range");
            return rsc.el[imod(k,ksize)]; 
          }

        double subpairing_score(int64_t a, int64_t b, bool_t circsub)
          { /* Argument consistency: */
            /* fprintf(stderr, "  sc a = %6lld  b = %6lld %c", a, b, (circsub ? 'C' : 'N')); */
        ...
        void store_candidate(int64_t r, int64_t s, bool_t circsub, double score)
          { /* Reduce the initial indices modulo the respective periods: */
            ...
            if (DEBLEV(2))
              { fprintf
                  ( stderr, 
                    "\n  perfect candidate starting at (%d,%d) steps = %lld score = %24.16e\n", 
            ...
            msm_cand_vec_insert(&cdv, &ncd, maxCands, &cd, frac);
          }
      }
  }
...
void msm_cand_scan_alignment
  ...
  )
  {
    ...
    for (ncol = 0; ncol < ng - 1; ncol++)
      { 
        /* fprintf(stderr, "\nlooking for candidates with r = %lld\n", rc); */
        /* fprintf(stderr, "  starting with [%lld,%lld]\n", rc, sc); */
        ...
        while ((sc < smax) && (vsma <= vbig)) 
          { sc++;
            /* fprintf(stderr, "  incrementing sc to [%lld,%lld]\n", rc, sc); */
            update_mxsma(rc,sc);
            vsma = (*(mxsma(rc,sc)));
            vbig = (*(mxbig(rc,sc)));
            /* fprintf(stderr, "  mxsma = %24.16e  mxbig = %24.16e\n", vsma, vbig); */
        ...
        if (ns <= minRungs)
          { assert(ns == minRungs);
            ...
            /* fprintf(stderr, "  incrementing rc and sc to [%lld,%lld]\n", rc, sc); */
            update_mxsma(rc,sc);
            update_mxbig(rc,sc);
          }
        else
          { rc++;
            /* fprintf(stderr, "  incrementing rc to [%lld,%lld]\n", rc, sc); */
            update_mxbig(rc,sc);
          }
      }
    ...
    void initialize_mxsma(int64_t ri, int64_t si)
      { int64_t rmax = si - minRungs + 1;
        ...
        /* fprintf(stderr, "  initializing mxsma(%lld..%lld,%lld)\n", ri, rmax, si); */
    ...
    void initialize_mxbig(int64_t ri, int64_t si)
      {
        ...
        /* fprintf(stderr, "  initializing mxbig(%lld,%lld..%lld)\n", ri, si, smax); */
      ...
      { assert(rn >= 0);
        ...
        for (rt = rmax; rt >= rn; rt--)
          { 
            ...
            /* fprintf(stderr, "  updating mxsma(%lld,%lld) = %24.16e\n", rt, sn, scmax); */
            (*(mxsma(rt,sn))) = scmax;
          }
    ...
    void check_mxsma(int64_t rn, int64_t sn)
      { assert(rn >= 0);
        ...
        int64_t rmax = sn - minRungs + 1; 
        assert(rmax >= rn);
        /* fprintf(stderr, "  checking mxsma(%lld..%lld,%lld)\n", rn, rmax, sn); */
        int64_t rt;
        for (rt = rmax; rt >= rn; rt--)
      ...
      { assert(rn >= 0);
        ...
        for (st = smax; st >= sn; st--)
          { 
            ...
            /* fprintf(stderr, "  updating mxbig(%lld,%lld) = %24.16e\n", rn, st, scmax); */
            (*(mxbig(rn,st))) = scmax;
            s = score(rn, st, FALSE);
            if (s > scmax) { scmax = s; }
          }
    ...
    void check_mxbig(int64_t rn, int64_t sn)
      { assert(rn >= 0);
        ...
        if ((! circp) && (smax >= ng)) { smax = ng - 1; }
        assert(smax >= sn);
        /* fprintf(stderr, "  checking mxbig(%lld,%lld..%lld)\n", rn, sn, smax); */
        int64_t st;
        for (st = smax; st >= sn; st--)
          { int64_t rmin = st - maxRungs + 1;
            if ((! circp) && (rmin < 0)) { rmin = 0; }
  ...
  { /* Allocate the candidate vector {cdv}: */
    ...
    for (i = 0; i < ncd; i++) 
      { /* Choose between circular and open pairing: */
        ...
        cdv.el[i] = msm_cand_throw(xp, yp, len, circP, atomP, diagP, 0.10);
      }
    return cdv;
  }
...
void msm_cand_vec_write(FILE *wr, msm_cand_vec_t *cdv)
  { int ncd = cdv->nel;
    ...
    fprintf(wr, "ncands = %d\n", ncd);
    for (i = 0; i < ncd; i++)
      { msm_cand_write(wr, NULL, &(cdv->el[i]), idSize, nameSize, ixSize, NULL);
        fputc('\n', wr);
      }
...
msm_cand_vec_t msm_cand_vec_read(FILE *rd)
  { /* Check and skip the file header: */
    ...
    int k; 
    for (k = 0; k < ncd; k++)
      { cdv.el[k] = msm_cand_read(rd, NULL, NULL); 
        fget_eol(rd);
      }
...
msm_cand_vec_t msm_cand_vec_map_to_finer(msm_cand_vec_t *cdv, int nxnew, int nynew, int nwtb)
  { fprintf(stderr, "  mapping %d candidates ...\n", cdv->nel);
    msm_cand_vec_t cdvnew = msm_cand_vec_new(cdv->nel);
    int ic;
    for (ic = 0; ic < cdv->nel; ic++)
      { cdvnew.el[ic] = msm_cand_map_to_finer(&(cdv->el[ic]), nxnew, nynew, nwtb); }
...
msm_cand_vec_t msm_cand_vec_interpolate(msm_cand_vec_t *cdv)
  { msm_cand_vec_t cdvnew = msm_cand_vec_new(cdv->nel);
    int ic;
    for (ic = 0; ic < cdv->nel; ic++)
      { cdvnew.el[ic] = msm_cand_interpolate(&(cdv->el[ic])); }
    return cdvnew;
  }
...
msm_cand_vec_t msm_cand_vec_refine
  ...
  { msm_cand_vec_t cdvnew = msm_cand_vec_new(cdvold->nel);
    int ic;
    int ncnew = 0; /* Number of candidates that were kept. */
    for (ic = 0; ic < cdvold->nel; ic++)
      { msm_cand_t *cdoi = &(cdvold->el[ic]);
        ...
        msm_cand_t cdni = msm_cand_refine
          ( cdoi, delta, kappa, shrink, maxunp, step_score, tb );
        msm_cand_vec_insert(&cdvnew, &ncnew, maxCands, &cdni, frac);
      }
...
void msm_cand_vec_insert
  ...
  )
  {
    ...
    k = (*ncandP);
    while ((k > 0) && (cdv->el[k-1].score < cd->score)) { k--; }
      ...
      { /* Discard {cd} if there is any better or equal cand that contains or is similar to {cd}: */
        ...
        for (j = 0; j < k; j++)
          { int ngnew = msm_pairing_num_rungs(cd->pr); /* Rungs in {cd}. */
            int ngboth = msm_cand_count_common_rungs(&(cdv->el[j]), cd); /* Shared rungs. */
        ...
        for (j = (*ncandP)-1; j >= k; j--)
          { int ngold = msm_pairing_num_rungs(cdv->el[j].pr); /* Rungs in {cdv->el[j]}. */
            int ngboth = msm_cand_count_common_rungs(&(cdv->el[j]), cd); /* Shared rungs. */
            if (ngboth >= frac*ngold) 
              { /* Discard candidate {cvd.el[j]}: */
                msm_cand_debug("S", &(cdv->el[j]));
                delete(j);
              }
          }
      }
    ...
    int bubble_up(int j)
      { while (j > 0)
          { /* Get parent: */
            ...
            if (cdv->el[k].score >= cdv->el[j].score) { break; }
            /* Swap parent with {cdv[j]}: */
            msm_cand_t pp = cdv->el[k]; cdv->el[k] = cdv->el[j]; cdv->el[j] = pp;
            j = k;
          }
        return j;
      }
    ...
    void delete(int j)
      { int i;
        for (i = j+1; i < (*ncandP); i++) { cdv->el[i-1] = cdv->el[i]; }
        (*ncandP)--;
      }
  }
