...
/* Last edited on 2008-01-11 21:20:46 by stolfi */
...
typedef struct msm_image_t
  { int NVX;
   ...
   /* A virtual image with {nix} by {niy} pixels, implemented as a
...
msm_image_t *msm_image_alloc(int NC, int NVX, int NVY);
  /* Allocates a virtual image with {NC} channels, {NVX} columns and
    {NVY} rows. The underlying {float_image}'s actual size is
    {ceil(NVX/scale)} by {ceil(NVY/scale)}, where {scale} is chosen so
...
/* PAINTING DOTS 
  ...
  where {xr,yr} are {x,y} reduced modulo {img->NVX,img->NVY}. */
...
void msm_image_normalize_and_write_as_pgm(msm_image_t *img, char *name, char *tag);
  /* Writes the image {img} to disk as a PGM file called "{name}{tag}.pgm".
    The entries of {img} are scaled so that the smallest pixel maps to gray,
    the largest pixel maps to black.  In particular, pixels with value
    {-INF} map to white. */
