...
/* Last edited on 2015-10-04 19:51:52 by stolfilocal */
...
double msm_double_vec_interpolate(double_vec_t *smp, double z)
  {
    double iz = floor(z);
    if (iz == z)
      { double *p = msm_double_vec_get_address(smp, (int)iz);
        return (*p);
      }
    else
      { double *p0 = msm_double_vec_get_address(smp, (int)iz);
        double *p1 = msm_double_vec_get_address(smp, (int)iz + 1);
