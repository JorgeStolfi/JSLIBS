/* See msm_pairing.h */
/* Last edited on 2018-03-04 22:58:05 by stolfilocal */
...
struct msm_pairing_t 
  { int ng;             /* Number of rungs in pairing. */
...
msm_pairing_t *msm_pairing_trim_to_span(msm_pairing_t *pr, int j, int span, int i, sign_t dir)
  { demand(span > 0, "cannot trim to zero span");
    ...
    int ini, fin; /* Indices of initial and final rung of segment in {pr}. */
...
msm_rung_vec_t msm_pairing_collect_rungs(msm_pairing_t *pr, int n0, int n1)
  { /* Get rung count {ngold} of original pairing: */
    int ng = msm_pairing_num_rungs(pr); /* Number of original rungs. */
    demand(ng >= 0, "invalid rung count");
    /* Reduced rung list is {gv_new[0..ng_new-1]}: */
    msm_rung_vec_t gv_new = msm_rung_vec_new(ng);
    int ng_new = 0; /* Number of distinct reduced rungs. */
    int i;
    for (i = 0; i < ng; i++)
          ...
          { /* _New rung, insert it in {gv_new}: */
            ...
            if (j < ng_new)
              { /* Open up the slot {gv_new[j]}: */
                int k = ng_new; while (k > j) { gv_new.e[k] = gv_new.e[k-1]; k--; }
              }
            gv_new.e[j] = g;
            ng_new++;
          }
...
void msm_pairing_enum_alignments
  ( int n0,       /* Number of positions on first sequence. */
    int n1,       /* Number of positions on second sequence. */ 
...
msm_pairing_t *msm_pairing_read(FILE *rd)
  {
    ...
    /* Initial rung: */
    msm_rung_t gini;
    gini.c[0] = fget_int(rd); demand(gini.c[0] >= 0, "bad init index for sequence 0"); 
    gini.c[1] = fget_int(rd); demand(gini.c[1] >= 0, "bad init index for sequence 1");
    if (ng == 0)
    ...
    if (ch == '*')
      ...
      { /* Generic pairing. */
        ...
        int kg = 0; /* Next free position in {gv}. */
        while (TRUE)
