...
/* Last edited on 2014-07-29 14:48:22 by stolfilocal */
...
typedef struct options_t 
  { int maxLevel;       /* Maximum filtering level. */
    int numPoints;      /* Number of bases in test sequence. */
...
void dnae_compute_random_dna_spectrum(int N, double dnaP[]);
...
void dnae_plot_spectra
  ...
  { /* Colors for curves (all with brightness = 0.30): */
    ...
    msm_ps_tools_draw_scale(dp, HOR, yMin, tickSize,0.0, 5.0,1.0, 0.5,0.5,0.5, "%.0f",  1.2, 12.0,0.0);
    msm_ps_tools_draw_scale(dp, VER, xMin, tickSize,0.0, 3.0,0.0, 0.5,0.5,0.5, "%+.2f", 1.2,  4.0,0.0);
...
options_t* dnae_get_options(int argc, char**argv)
  { options_t *o = (options_t *)notnull(malloc(sizeof(options_t)), "no mem");
    ...
    o->numPoints = (int)argparser_get_next_int(pp, 0, (1 << 18));
