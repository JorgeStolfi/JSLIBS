/* See pswr.h */
/* Last edited on 2019-05-06 20:02:28 by jstolfi */
...
void pswr_write_proc_defs(FILE *psFile, double hSize, double vSize);
  /* Writes to {psFile} the definition of the operators assumed
    by other plotting and text writing commands.  Also writes the
    definition of the Postscript variables {pt,in,mm} (number of 
    points in one point, inch, and millimeter, respectively) 
    and {hSize,vSize} to the values give.  These definitons
    are placed in the current Postscript dictionary. */
...
void pswr_begin_encapsulated_figure(PSStream *ps)
  { affirm(ps->eps, "bad file type");
    ...
    FILE *file = ps->file;
    char *date = today();
    pswr_write_eps_file_header(file, date, ps->hCanvasSize, ps->vCanvasSize);
...
void pswr_begin_document(PSStream *ps)
  { affirm(! ps->eps, "bad file type");
    ...
    char *date = today();
    if (ps->verbose) { fprintf(stderr, "date = %s\n", date); }
    pswr_write_ps_file_header(file, ps->paperSize, ps->landscape, ps->hCanvasSize, ps->vCanvasSize, date);
    fflush(file);
    free(date);
  }
...
void pswr_write_eps_file_header
  ...
  { fprintf(file, 
    ...
    pswr_write_proc_defs(file, hSize, vSize);
    fprintf(file, 
      "end\n"
    );
...
void pswr_write_ps_file_header
  ...
  { fprintf(file, 
    ...
    if (landscape) 
      { fprintf(file, 
          " -90 rotate hSize 144 sub 0 translate\n"
        );
      }
...
void pswr_write_ps_string(FILE *file, const char *text, const char *newline)
  { const unsigned char *p;
    putc('(', file);
    for (p = (unsigned char *)text; *p != 0; p++)
      { if (*p == '\n')
...
void pswr_write_proc_defs(FILE *file, double hSize, double vSize)
  { 
    ...
    fprintf(file, 
      ...
      "        { 1 eq { gsave fR fG fB setrgbcolor fill grestore } if\n"
      "          1 eq { stroke } if\n"
      "        }\n"
      "      ifelse\n"
      "    }\n"
    ...
    fprintf(file, 
      ...
      "        { 1 eq { gsave fR fG fB setrgbcolor eofill grestore } if\n"
      "          1 eq { stroke } if\n"
      "        }\n"
      "      ifelse\n"
      "    }\n"
    ...
    fprintf(file, 
      ...
      "  gsave\n"
      "    %% --- str, xa, ya\n"
      "  2 index false charpath flattenpath pathbbox\n"
      "  grestore\n"
     ...
     fprintf(file, 
      "%% Label fill and stroke operator:\n"
      "%%   {draw} {fill} {str} {xa} {ya} {rot} {xc} {yc} lbsh --> \n"
      "/lbfs\n"
      "{ labelfont setfont\n"
      "  newpath moveto\n"
