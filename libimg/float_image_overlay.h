...
/* Last edited on 2021-08-25 10:45:41 by stolfi */ 
...
void float_image_overlay(float_image_t *A, float_image_t *B, int32_t icop);
  /* Overlays image {B} on top of image {A}, assuming that channel {icop}
    of both is the opacity channel.
    
    The two images must have the same number of columns, rows, and channels.
    
    For each pixel and each channel other than {icop}, let {vA,vB} be
    the samples of {A} and {B} from that pixel and channel. The
    procedure sets {vA} to be {(oB*vB + (1-oB)*oA*vA)/oC} where {oC = 1
    - (1-oA)*(1-oB) = oB + (1-oB)*oA}; then sets {oA} to {oC}.
