...
void foo(void)
  {
    ...
    for (i = 0; i < n; i++) 
      { /* Check channel count of {img[i]}: */
        demand(nc <= img[i]->sz[0], "invalid channel count");
      }
    ...
    for (k = 0; k < 2; k++)
      { for (i = 0; i < n; i++) 
        ...
        demand(nr.c[k] >= 0, "window radius must be non-negative");
      }


    /* Compute weights of window points: */
...
void fial_align_var
  ( int n,
    ...
    i2_t nr,                /* Half-extent of comparison window along each axis. */
    double *QP              /* (OUT) Discrepancy measure for best alignment. */
  )
  { 
    bool_t debug = TRUE;
    ...
    /* Check search grid parameters: */
    for (k = 0; k < 2; k++)
      { demand(ns.c[k] >= 0, "search step count must be non-negative");
        if (ns.c[k] > 0)
    ...
    i2_t d[n];          /* Adjustment step counts to apply to {p[0..n-1]}. */
    i2_t dbest[n];      /* Best step count tuple found so far. */
    double Qbest = INF; /* Mismatch of best alignment. */

    r2_t q[n];      /* Work: Displaced points {q[i] = p[i] + d[i]}. */
        
    /* Initialize the adjustment step counts {d[0..n-1]}: */
    for (i = 0; i < n; i++) { d[i] = (i2_t){{0,0}}; }
    
    /* Enumeration loop: */
    while (1) 
      { /* Evaluate {Q} for the adjustment step count tuple {d[0..n-1]}: */
        fial_displace_points(n, p, d, q, step); 
        double Q = fial_mismatch_var(n, img, q, nc, nr, w0, w1);
        if (debug) { fial_debug_displacement("trial", n, d, q, Q, "\n"); }
        if (Q < Qbest) { Qbest = Q; for (i = 0; i < n; i++) { dbest[i] = d[i]; } } 
        /* Advance to the next tuple: */
        if (fial_next_tuple(n, d, ns)) 
          { /* Enumeration finished, return the best solution: */
            fial_displace_points(n, p, dbest, p, step); 
            (*QP) = Qbest;
            return;
          }
      }
