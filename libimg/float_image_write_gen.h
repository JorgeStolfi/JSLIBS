...
/* Each procedure in this section writes given {float_image_t} in-memory image
  ...
  The scaled samples are then gamma-encoded with exponent {gammaEnc} and
  bias {bias} before quantization. See {sample_conv_gamma}. If
  {gammaEnc} is {NAN}, uses a the ITU-R BT709 encoding exponent
  {sample_conv_BT709_ENC_GAMMA} = 0.45. If {bias} is {NAN}, uses the
  BT709 value {sample_conv_BT709_BIAS = 0.0327}. To suppress gamma
