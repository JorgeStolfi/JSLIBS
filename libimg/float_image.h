...
void float_image_write_named(char *fname, float_image_t *A);
  /* Same as {float_image_write}, but writes to a new file called
    "{fname}" instead of a file descriptor. The recommended extension is
    ".fni". */
...
float_image_t *float_image_read_named(char *fname);
  /* Same as {}, but reads from an existing file called "{fname}"
    instead of a file descripto. The recommended extension is ".fni". */
