  ...
  { if (shrink != 0)
      { /* Shrink the colors towards their average: */
        ...
        for(int32_t c = 0; c < CHNS; c ++)
          { double bj = 0;
            for (uint32_t i = 0;  i < LAYS; i++) { bj += mix->c[i][c]; }
            bj /= LAYS;
            for (uint32_t i = 0;  i < LAYS; i++) { mix->c[i][c] = t*mix->c[i][c] + s*bj; }
          }
      }
  }
...
void frgb_inksep_separate_layers(frgb_t fv, r4x4_t *sep, r4_t *m)
  { /* Convert color {fv[0..chns-1]} into RGBW homogeneous quadruple: */
    r4_t y;
    for (uint32_t c = 0;  c < LAYS; c++) { y.c[c] = fv.c[c]; }
    y.c[LAYS-1] = 1.0;
...
frgb_t frgb_inksep_remap_color(r4_t *m, r4x4_t *syn)
  {
    ...
    frgb_t sv;
    for (uint32_t c = 0;  c < CHNS; c++) { sv.c[c] = (float)(sh.c[c]/sh.c[3]); }
    return sv;
  }
