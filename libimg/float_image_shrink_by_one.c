/* See float_image_shrink_by_one.h */
/* Last edited on 2025-01-18 17:35:48 by stolfi */ 
...
#include <float_image_shrink_by_one.h>
            ...
            for (int32_t c = 0; c < NCA; c++)
              { if (c != cW) 
                  { double aexp = (sum_w == 0 ? 0.0 : sum_wa[c]/sum_w);
                    float_image_set_sample(R, c, x, y, (float)aexp);
                  }
                else
                  { double wexp = (num_w == 0 ? 0.0 : num_w/sum_m); /* Harmonic mean. */
                    float_image_set_sample(R, c, x, y, (float)wexp);
                  }
              }
          }
      }
