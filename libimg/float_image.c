...
void float_image_set_channel(float_image_t *A, int32_t cA, float_image_t *V, int32_t cV)
  { int32_t NCV, NX, NY;
    float_image_get_size(V, &NCV, &NX, &NY);
...
void float_image_mix_channels
  ...
  { int32_t NCR,  NX, NY;
    float_image_get_size(R, &NCR, &NX, &NY);
    int32_t NCA = (int32_t)A->sz[0];
...
void float_image_local_avg_var
  ...
  )
  {
    ...
    int32_t NCV, NXV, NYV;
    if (V != NULL)
      { float_image_get_size(V, &NCV, &NXV, &NYV);
        demand((cV >= 0) && (cV < NCV), "invalid {V} channel");
      }
    else
      { NCV = NXV = NYV = 0; }
    ...
    int32_t NX = (int32_t)imax((int32_t)NXM, (int32_t)NXV);
    int32_t NY = (int32_t)imax((int32_t)NYM, (int32_t)NYV);
...
void float_image_apply_gamma(float_image_t *A, int32_t c, double expo, double bias)
  { if (expo == 1) { return; }
    demand(expo > 0, "gamma transform {expo} must be positive"); 
    int32_t NC = (int32_t)A->sz[0];
    int32_t NX = (int32_t)A->sz[1];
    int32_t NY = (int32_t)A->sz[2];
    ...
    for (int32_t y = 0;  y < NY; y++)
      { for (int32_t x = 0;  x < NX; x++)
          { float *pA = float_image_get_sample_address(A, c, x, y);
            (*pA) = sample_conv_gamma((*pA), expo, bias);
          }
      }
  }
...
float_image_t *float_image_new (int32_t NC, int32_t NX, int32_t NY)
  { /* Sanity checks: */
    ...
    ix_count_t NS = ((ix_count_t)NC)*((ix_count_t)NX)*((ix_count_t)NY);
    ...
    A->sz[0] = (ix_size_t)NC; A->st[0] = (NC <= 1 ? 0 : 1);
    A->sz[1] = (ix_size_t)NX; A->st[1] = (NX <= 1 ? 0 : (ix_step_t)NC);
    A->sz[2] = (ix_size_t)NY; A->st[2] = (NY <= 1 ? 0 : (ix_step_t)(NC*NX));
    A->bp = 0;
    if ((NC == 0) || (NX == 0) || (NY == 0))
      { A->sample = (float *)NULL; }
    else
