...
void float_image_set_sample_row
  ...
  { if (xmin <= xmax)
      { int32_t nrx = xmax - xmin + 1; /* Count of samples to process. */
        ...
        for (uint32_t ix = 0;  ix < nrx; ix++) { (*sp) = v[ix]; sp += A->st[1]; }
      }
  }
...
void float_image_fill_sample_row 
  ...
  { if (xmin <= xmax)
      { int32_t nrx = xmax - xmin + 1; /* Count of samples to process. */
        /* Get pointer {sp} to next sample to set: */
        float *sp = float_image_get_sample_address(A, c, xmin, y);
        for (uint32_t ix = 0;  ix < nrx; ix++) { (*sp) = v; sp += A->st[1]; }
      }
  }
...
void float_image_fill(float_image_t *A, float v)
  { int32_t NC = (int32_t)A->sz[0];
    for (uint32_t c = 0;  c < NC; c++) { float_image_fill_channel(A, c, v); }
  }

void float_image_fill_pixels(float_image_t *A, float v[])
  { int32_t NC = (int32_t)A->sz[0];
    for (uint32_t c = 0;  c < NC; c++) { float_image_fill_channel(A, c, v[c]); }
  }

void float_image_assign(float_image_t *A, float_image_t *V)
  { int32_t NC = (int32_t)A->sz[0];
    demand(V->sz[0] == NC, "incompatible channel counts");
    for (uint32_t c = 0;  c < NC; c++) { float_image_set_channel(A, c, V, c); }
  }
    ...
    for (int32_t y = ymin; y <= ymax; y++)
      { float *ppix = prow;
        for (int32_t x = xmin; x <= xmax; x++)
          { float *psmp = ppix;
            for (uint32_t c = 0;  c < A->sz[0]; c++) { (*psmp) = v; psmp += A->st[0]; }
            ppix += A->st[1];
          }
        prow += A->st[2];
      }
  ...
  { if ((xmin > xmax) || (ymin > ymax)) { return; }
    ...
    for (int32_t y = ymin; y <= ymax; y++)
      { float *ppix = prow;
        for (int32_t x = xmin; x <= xmax; x++)
          { float *psmp = ppix;
            for (uint32_t c = 0;  c < A->sz[0]; c++) { (*psmp) = v[c]; psmp += A->st[0]; }
            ppix += A->st[1];
          }
        prow += A->st[2];
      }
    ...
    for (uint32_t y = 0;  y < NY; y++)
      { for (uint32_t x = 0;  x < NX; x++)
          { frgb_t pix = fic_get_frgb_pixel(A, 0, 1, 2, x, y);
            ...
            for (uint32_t c = 0;  c < NC; c++) { pix.c[c] = (float)value; }
...
float_image_t *float_image_crop
  ...
  { /* Grab the dimensions of {A}: */
    ...
    for (uint32_t c = 0;  c < NCR; c++)
      { int32_t cA = cLo + c;
        bool_t cOK = ((cA >= 0) && (cA < NCA));
        for (uint32_t x = 0;  x < NXR; x++)
          { int32_t xA = xLo + x;
            bool_t xOK = ((xA >= 0) && (xA < NXA));
            for (uint32_t y = 0;  y < NYR; y++)
              { int32_t yA = yLo + y;
...
void float_image_debug_pixel(char *label, double x, double y, int32_t chns, float f[], char *tail)
  { 
    ...
    for (uint32_t ich = 0;  ich < chns; ich++) 
