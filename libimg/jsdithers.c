...
/* These ordered dithering matrices are taken from "Digital
  Halftoning" by Robert Ulichney, MIT Press, ISBN 0-262-21009-6. */

/* Order-6 scattered dithering matrix. */
uint8_t dither_8x8[64] = 
  (uint8_t[64]) {
...
/* Order-8 scattered dithering matrix. */
static uint8_t dither_16x16[256] =
  (uint8_t[256]) {
...
static uint8_t cluster_16x16[256] = 
  (uint8_t[256]) {
...
uint8_t *get_dither_matrix(int32_t n, bool_t cluster)
  {
    if (cluster)
      { if (n == 6)
          ...
          { return cluster_16x16; }
        else 
          { demand(FALSE, "invalid clustered dither matrix size"); }
      }
    else
      { if (n == 8)
          { return dither_8x8; }
          ...
          { demand(FALSE, "invalid scattered dither matrix size"); }
      }
  }
