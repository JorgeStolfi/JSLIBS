...
/* Last edited on 2017-06-25 02:50:41 by stolfilocal */
...
void float_image_geostereo_get_samples
  ( float_image_t *f,   /* Pixel row buffer for image 1. */
    double x,           /* Central column (origin for displacement). */
    int32_t y,          /* Current row index in image. */
    double d,           /* The displacement (pixels, fractional). */
    int rx,             /* Window half-width. */
    int ry,             /* Window half-height. */
    float w[]           /* (OUT) Window sample buffer. */
  );
  /* Extracts samples from a window with dimensions {[-rx..+rx]} by {[-ry..+ry]}
    centered on row {y} at point {x-d} and {x+d}, respectively.
