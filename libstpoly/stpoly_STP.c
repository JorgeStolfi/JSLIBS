/* See {stpoly_STP.h} */
/* Last edited on 2022-10-20 05:59:59 by stolfi */
...
bool_t stpoly_STP_skip_white_space(FILE *rd, char *fileName, int32_t *lineP);
...
char *stpoly_STP_get_keyword(FILE *rd, char *fileName, int32_t *lineP);
...
void stpoly_STP_check_keyword(FILE *rd, char *fileName, int32_t *lineP, char *key);
...
bool_t stpoly_STP_ascii_read_edge(FILE *rd, char *fileName, int32_t *lineP, stpoly_STP_edge_t *edge);
...
int32_t stpoly_STP_binary_read_header(FILE *rd, char *fileName, int32_t *lineP);
...
void stpoly_STP_binary_read_edge(FILE *rd, char *fileName, int32_t *lineP, stpoly_STP_edge_t *edge);
...
void stpoly_STP_read(char *fileName, bool_t binary, stpoly_STP_edge_proc_t *process_edge)  
  {
    ...
    int32_t ne = 0;   /* Number of segs read from the STP file. */
    
    if (binary)
      { int32_t ne = stpoly_STP_binary_read_header(rd, fileName, &line);
        int32_t it;
        for (it = 0; it < ne; it++) 
...
int32_t stpoly_STP_binary_read_header(FILE *rd, char *fileName, int32_t *lineP)
  { 
    ...
    (*lineP)++;
    err = (int32_t)fread((void*)(&ne), 4, 1, rd);
