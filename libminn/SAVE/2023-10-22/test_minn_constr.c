#define PROG_NAME "test_minn_constr"
#define PROG_DESC "test of {minn_constr.h}"
#define PROG_VERS "1.0"

/* Last edited on 2023-03-24 12:33:43 by stolfi */ 
/* Created on 2007-07-11 by J. Stolfi, UNICAMP */

#define tmnc_COPYRIGHT \
...
#include <test_minn_constr_info.h>
#include <test_minn_constr_basic.h>
#include <test_minn_constr_plot.h>
#include <test_minn_constr_tools.h>

/* #include <float_image.h> */
/* #include <float_image_interpolate.h> */
/* #include <float_image_io_pnm.h> */

#define tmnc_MAX_SCALE (15)
  /* Hopefully the scale will never be higher than this. */ 

void tmnc_do_one_test
  ( char *fname, 
    bool_t bias, 
    bool_t monoscale, 
    bool_t quadopt, 
...
void tmnc_choose_optimum(int32_t NI, r2_t pini[], r2_t arad[], r2_t popt[]);
...
double tmnc_compute_image_mismatch_sqr
  ( int32_t NI,                       /* Number of images being compared. */
    tmnc_image_eval_proc_t *eval, /* Image evaluator. */
...
void tmnc_compute_avg_var(int32_t nz, double z[], double *avgP, double *varP);
...
int32_t main(int32_t argc, char **argv)
  {
    ...
    tmnc_do_one_test(fname, bias, monoscale, quadopt, NI, cmp_isc, adj_rad, adj_stp);
    return 0;
  }
...
void tmnc_do_one_test
  ...
  { 
    ...
    i2_t wsize; 
    double *wtx; tmnc_make_weight_table(cmp_rad.c[0], &wsize.c[0], &wtx);
    double *wty; tmnc_make_weight_table(cmp_rad.c[0], &wsize.c[1], &wty);
    ...
    tmnc_choose_pini_arad_astp(NI, adj_rad, adj_stp, pini, arad, astp);
    ...
    if (strcmp(fname, "indiff") == 0)
      ...
      { f2_raw = &f2_optdst;
        ...
        tmnc_choose_optimum(NI, pini, arad, popt);
      ...
      { f2_raw = &f2_imgmis;
        ...
        tmnc_tools_initialize_mother_image(mom_NF, mom_frq, mom_phi, mom_amp, TRUE);
        ...
        for (int32_t isc = -1; isc < 3; isc++)
          { i2_t ipscale = ( isc < 0 ? iscale : (i2_t){{ isc, isc }} ); /* Plotting scale. */
            for (int32_t i = 0; i < NI; i++)
              { tmnc_write_test_image(image_eval, i, ipscale, cmp_rad, pini[i], "ini", arad[i]);
                tmnc_write_test_image(image_eval, i, ipscale, cmp_rad, popt[i], "opt", arad[i]);
              }
          }
      }
    else 
    ...
    tmnc_debug_points(0, "initial guess ", NI, "pini", pini, NULL, NULL, arad, astp, f2ini, b2ini);
    ...
    tmnc_debug_points(0, "actual optimum", NI, "popt", popt, pini, NULL, arad, astp, f2opt, b2opt);
    ...
    for (int32_t isc = -1; isc < 3; isc++)
      { i2_t ipscale = ( isc < 0 ? iscale : (i2_t){{ isc, isc }} ); /* Plotting scale. */
        ...
        nf2 = 0;
        tmnc_plot_goal(f2_full, NI, ipscale, pini, "ini", arad);
        fprintf(stderr, "%d calls to {f2_full} for plotting.\n", nf2);
        nf2 = 0;
        tmnc_plot_goal(f2_full, NI, ipscale, popt, "opt", arad);
        fprintf(stderr, "%d calls to {f2_full} for plotting.\n", nf2);
      }
    ...
    tmnc_debug_points(0, "computed optimum", NI, "psol", psol, pini, popt, arad, astp, f2_raw_sol, bias_sol);
    assert(f2sol == f2_raw_sol + bias_sol);
    
    /* Print again raw function value and bias term for optimum point: */
    tmnc_debug_points(0, "actual optimum", NI, "popt", popt, pini, NULL, arad, astp, f2opt, b2opt);
    
    bool_t ok = tmnc_ckeck_result(NI, psol, f2sol, pini, (compare_to_popt ? popt : NULL), arad, astp);
      ...
      { demand(ni == NI, "duh?");
        ...
        if (debug_points) 
          { int32_t ind = 2*(int32_t)imax(iscale.c[0], iscale.c[1]); /* Indentation. */
            tmnc_debug_points(ind, "probe point   ", ni, "pcur", p, pini, popt, arad, astp, f2p, b2p);
          }
        return f2p + b2p;
      }
    ...
    double f2_imgmis(int32_t ni, r2_t p[], i2_t iscale)
      { double f2p = tmnc_compute_image_mismatch_sqr
    ...
    double image_eval(int32_t i, i2_t iscale, double xsc, double ysc)
      {
        ...
        return tmnc_tools_eval_mother_image(&psc, &scale, mom_NF, mom_frq, mom_phi, mom_amp);
      }
...
void tmnc_choose_pini_arad_astp
  ...
  )   
  {
    ...
    tmnc_debug_params(0, "rad", NI, arad);
    tmnc_debug_params(0, "stp", NI, astp);
  }
    ...
    for (int32_t jy = -hwy; jy <= +hwy; jy++)
      { for (int32_t jx = -hwx; jx <= +hwx; jx++)
          { /* Sample the images at this grid sampling point: */
            ...
            double avg, var;
            tmnc_compute_avg_var(NI, val, &avg, &var);
...
void minn_constr_throw_arad(int32_t n, double zfrac, double rmin, double rmax, double rad[])
  { 
    for (int32_t j = 0; j < 2; j++)
      { /* Decide how many coordinates {rad[0..n-1].c[j]} will be set to zero. */
        int32_t nzgoal = (int32_t)floor(zfrac.c[j]*n + 0.5); 
        if (n >= 2)
          { /* Cook {nzgoal} to be {0} or {n} iff {zfrac} is 0.0 or 1.0, resp.: */
            if ((nzgoal ==  0) && (zfrac.c[j] > 0.0)) { nzgoal = 1; }
            if ((nzgoal == n) && (zfrac.c[j] < 1.0)) { nzgoal = n-1; }
          }
        assert(nzgoal <= n);
        int32_t nc = n;      /* Number of coordinates remaining. */
