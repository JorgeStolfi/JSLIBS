/* voxb_splat_tube.h --- voxel-based modeling of tubes and ropes */
/* Last edited on 2021-06-09 23:21:59 by jstolfi */

#ifndef voxb_splat_tube_H
#define voxb_splat_tube_H
...
/* SPLATTING TUBES, ROPES, ETC 
  ...
  Each procedure in this section splats into the tomogram {a} 
  an object obtained by sweeping a /brush/ along a path.
  ...
  If {sub} is false, the brush is a torus with outer radius {otR} and
  hole radius {inR}, so that the worm is a tube with outer radius {otR}
  and inner radius {inR}. The tube is united (splatted with
  {voxb_op_OR}) with the objects already in {a}.
  
  If {sub} is true, the brush is a sphere with radius {inR}, so that the
  worm is a wire with radius {inR}. The wire is subtracted (splatted
  with {voxb_op_SUB}) from the objects already in {a}. This is useful to
  ensure that the bore of a tube is clear even if it crosses other
  objects. */
...
void voxb_splat_tube_round_helix
  ...
  /* The midline is an arc of helix,  generated by {r3_motion_helix(t,L,A,H)} 
