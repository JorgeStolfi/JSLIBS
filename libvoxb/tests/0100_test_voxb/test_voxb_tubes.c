...
/* Last edited on 2022-10-20 05:48:55 by stolfi */
...
void test_voxb_tubes(ppv_array_t *A, r3_t *ctr, r3_t *rad)
  { 
    ...
    bool_t doit[NT];
    doit[0] =  TRUE; /* {test_voxb_tubes_helix}. */
    doit[1] =  TRUE; /* {test_voxb_tubes_segment}. */
    doit[2] =  TRUE; /* {test_voxb_tubes_bezier}. */
...
void test_voxb_tubes_helix(ppv_array_t *A, r3_t *ctr, r3_t *rad)
  { 
    ...
    r3_t shift = (*ctr);
    for (k = 0; k < N; k++)
      { test_voxb_rescale_r3_motion_state(&(S[k]), scale, &shift); }
...
void test_voxb_tubes_segment(ppv_array_t *A, r3_t *ctr, r3_t *rad)
  { 
    ...
    for (k = 0; k < N; k++)
      { S[k].t = 0.00;
        test_voxb_rescale_r3_path_state(&(S[k]), scale, &shift);
        T[k].t = 1.00;
        test_voxb_rescale_r3_path_state(&(T[k]), scale, &shift);
      }
...
void test_voxb_tubes_bezier(ppv_array_t *A, r3_t *ctr, r3_t *rad)
  { 
    ...
    int32_t k;
    for (k = 0; k < N; k++)
      { test_voxb_rescale_r3(&(p0[k]), scale, &shift);
        test_voxb_rescale_r3(&(p1[k]), scale, &shift);
        test_voxb_rescale_r3(&(p2[k]), scale, &shift);
        test_voxb_rescale_r3(&(p3[k]), scale, &shift);
      }
