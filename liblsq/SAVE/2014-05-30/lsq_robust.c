/* See {lsq_robust.h} */
/* Last edited on 2014-05-30 12:32:16 by stolfilocal */
...
void lsq_robust_fit
  ...
  )
  {
    ...
    int nf = 1; /* Number of dependent variables. */
        ...
        for (iter = 1; iter <= maxiter; iter++)
          {
            ...
            /* Set {Fc} to the deviations from approximation: */
            rn_sub(nt, F, Fa, Fc);
            ...
            for (k = 0; k < nt; k++)
              { /* Decide the probability {Pc[k]} of each data record {X[k,*},F[k,*]} being an inlier: */
                /* Grab the sample value vector {Fk} and current approximation {Fak}: */
                double Fk = F[k];
                double Fak = Fa[k];
...
void lsq_robust_compute_stats
  ...
  )
  {
    ...
    if (verbose)
      { char* tag = ((char*[2]){ "bad", "gud" })[inlier];
        lsq_robust_debug_distr(stderr, tag, avg, dev, pri);
      }
    /* Return: */
    (*avgP) = avg;
    (*devP) = dev;
...
double lsq_robust_bayes
  ...
  {
    /* Compute the squared discrepancy relative to the inlier and outlier distrs: */
    double d_gud = (F - avg_gud)/dev_gud; /* Displacement of {F} from inlier average. */
    if (d_gud > 6.0) { return 0.0; }
    double d_bad = (F - avg_bad)/dev_bad; /* Displacement of {F} from outlier average. */
    if (d_bad > 6.0) { return 1.0; }

    /* Apply Bayes's formula: */
    double pri_bad = 1 - pri_gud;
    double Pc_gud = exp(-d_gud*d_gud/2)/dev_gud; /* Prob of func values times {sqrt(2*PI)}, assuming inlier. */
    double Pc_bad = exp(-d_bad*d_bad/2)/dev_bad; /* Prob of func values times {sqrt(2*PI)}, assuming outlier. */
