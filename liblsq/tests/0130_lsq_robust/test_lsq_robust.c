...
/* Last edited on 2024-12-21 09:00:24 by stolfi */
...
void test_lsq_robust_fit(uint32_t g, uint32_t ixpoly, double dev_gud, double pri_bad, double dev_bad, bool_t verbose);
...
void test_lsq_robust_throw_poly(uint32_t g, uint32_t ixpoly, double R[]);
...
void test_lsq_robust_gen_poly_bezier(uint32_t g, uint32_t ix, double a, double b, double R[]);
  /* Sets {R[0..g]} to the coefficients of the Bernstein-Bezier polynomial
    of degree {g} and index {ix} for the domain interval {[a_b]}. */

void test_lsq_robust_gen_args(uint32_t nt, double x[]);
  /* Fills {x[0..nt-1]} with equally spaced values in {[-1 _ +1]}. */ 

double test_lsq_robust_poly_eval(uint32_t g, double R[], double x);
  /* Evaluates the polynomial with coeffs {R[0..g]} at the given {x} value. */
  
void test_lsq_robust_poly_eval_multi(uint32_t g, double R[], uint32_t nt, double x[], double yt[]);
  /* Assumes that {x} and {yt} have {nt} elements. Sets {yt[k]} to the
    value of the polynomial with coeffs {R[0..g]} at the argument
    {x[k]}, for {k} in {0..nt-1]}. */

void test_lsq_robust_gen_weights(uint32_t nt, double x[], double w[]);
...
void test_lsq_robust_print_data
  ( char *name,
    uint32_t g,       /* Degree of target polynomial. */
    uint32_t ixpoly,  /* Index of target polynomial. */
    int32_t iter,     /* Iteration count, or {-1} for the target and final states. */
    uint32_t nt,      /* Number of sampling points. */
...
void test_lsq_robust_fill_basis_matrix(uint32_t nt, double x[], uint32_t g, double X[]);
...
void test_lsq_robust_check_fit(uint32_t nt, double yt[], double yf[], double dev_gud, double pri_bad);
...
int32_t main (int32_t argc, char **argv)
  { 
    uint32_t iarg = 1;
    uint32_t g = (uint32_t)atoi(argv[iarg]); iarg++; /* Degree of polynomial. */
    uint32_t ixpoly = (uint32_t)atoi(argv[iarg]); iarg++; /* Polynomial index within degree {g}, from 0. */
...
void test_lsq_robust_fit(uint32_t g, uint32_t ixpoly, double dev_gud, double pri_bad, double dev_bad, bool_t verbose)
  { 
    uint32_t nt = N_POINTS;  /* Num of data points. */
    uint32_t nv = g+1;       /* Num of coefficients to fit. */
    ...
    { auto void report(uint32_t iter, uint32_t ntc, uint32_t nvc, double Pc[], double yc[], double Qc[]);
      ...
      void report(uint32_t iter, uint32_t ntc, uint32_t nvc, double Pc[], double yc[], double Qc[])
        { assert(ntc == nt);
          assert(nvc == nv);
          double yf[nt];  /* Current approximation evaluated at sampling arguments. */
          test_lsq_robust_poly_eval_multi(g, Qc, nt, x, yf);
          test_lsq_robust_print_data("iter", g, ixpoly, (int32_t)iter, nt, x, Pc, yc, yf, "%+12.6f", verbose);
        }
    }
...
void test_lsq_robust_gen_poly_bezier(uint32_t g, uint32_t ix, double a, double b, double R[])
  {
    ...
    for (int32_t k = (int32_t)ix; k < g; k++)
...
double test_lsq_robust_poly_eval(uint32_t g, double R[], double x)
  {
    ...
    double y = R[g];
    for (int32_t i = (int32_t)g-1; i >= 0; i--) { y = R[i] + x*y; }
    return y;
  }
...
void test_lsq_robust_gen_args(uint32_t nt, double x[])
  { for(uint32_t k = 0; k < nt; k++) { x[k] = 2*(k + 0.5)/nt - 1.0; }
  }
...
void test_lsq_robust_print_data
  ...
  { char *fname = NULL;
    if (iter >=0)
      { fname = jsprintf("out/%s_g%02d_ix%03d_i%03d.txt", name, g, ixpoly, iter); }
    else
      { fname = jsprintf("out/%s_g%02d_ix%03d.txt", name, g, ixpoly); }
