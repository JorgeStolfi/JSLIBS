...
/* Last edited on 2019-12-18 20:30:05 by jstolfi */
...
void test_lsq_fit(int trial, double eps, bool_t verbose);
  /* Tests the least-squares fitter with noise magnitude {eps}. */

void test_lsq_throw_linear_fn(int nx, int nf, double M[]);
  /* Generates a random {nx × nf} linear transformation matrix {M}. */

void test_lsq_throw_data_point(int nx, int nf, double M[], double eps, double v[], double f[]);
...
void test_lsq_check_fit(int nx, int nf, double M[], double eps, double U[]);
...
void test_lsq_fit(int trial, double eps, bool_t verbose)
  { 
    ...
    int nx = rand()/(RAND_MAX/MAX_VARS) + 1; /* Number of independent variables (argument coords per data point). */
    int nf = rand()/(RAND_MAX/MAX_FUNS) + 1; /* Number of dependent variables (function samples per data point). */
    int nt = (int)imax(10000, 2*nx + 10);    /* Number of data points. */
    ...
    auto void gen_data_point(int k, int nxg, double Xkg[], int nfg, double Fkg[], double *WkgP);
    void gen_data_point(int k, int nxg, double Xkg[], int nfg, double Fkg[], double *WkgP)
      { 
        assert(nxg == nx);
        assert(nfg == nf);
        assert((k >= 0) && (k < nt));
    ...
    int rank; /* Rank of least squares system. */
