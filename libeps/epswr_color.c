      ...
      { int32_t k = kMin + i;  /* Index of upper isoline of band {i}. */
        ...
        if (i == 0) 
          ...
          { /* Function values at lower and upper isolines of band: */
            double vLo = epswr_level(vStart, vStep, k-1);
            double vHi = epswr_level(vStart, vStep, k);
        ...
        if (r > 0.0)
          { epswr_interpolate_colors(+r, RZer,GZer,BZer, RMax,GMax,BMax, Ri,Gi,Bi); }
        else
          { epswr_interpolate_colors(-r, RZer,GZer,BZer, RMin,GMin,BMin, Ri,Gi,Bi); }
      }
