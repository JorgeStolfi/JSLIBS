...
/* Last edited on 2024-11-23 18:54:14 by stolfi */
...
void rmxn_canonical_simplex_throw(uint32_t d, double x[])
  { /* Generate a random point in the unit cube {[0_1]^d}: */
    for (uint32_t i = 0;  i < d; i++) { x[i] = drandom(); }
    ...
    while (i > 0)
      { x[i] = x[i] - x[i-1];
        i--; /* Safe here since {i > 0} */
      }
    return;
    
    int32_t cmp(const void *a, const void *b) 
      { return cmp_double((double *)a, (double *)b); }
  }
...
void rmxn_canonical_simplex_ball_throw(uint32_t d, double x[]) 
  { /* Generate a random point in the unit {(d+1)}-dimensional ball: */
    ...
    double sum = 0.0;
    for (uint32_t i = 0;  i <= d; i++) { sum += x[i]; }
    ...
    if (rx == 0) { rx = 1; }
    double q = sum/(d+1);
    for (uint32_t i = 0;  i <= d; i++) { x[i] = (x[i] - q)/rx; }
