/* test_gauss_elim_triang.c --- test program for gauss_elim_triangularize.h  */
/* Last edited on 2024-11-26 20:11:04 by stolfi */
              ...
              { /* We have one more effective equation: */
                for (uint32_t k = 0; k < p; k++)
                  { double Xjk_ref = X_ref[j*p + k]; /* Intended solution. */
                    double Bik_exp = AB[i*np + n + k] * Xjk_ref; /* Expected {B}. */
                    double Xjk_cmp = X[j*p + k]; /* Solution by library. */
                    double Bik_cmp = AB[i*np + n + k] * Xjk_cmp; /* Its {B}. */
                    /* The residual must be zero or nearly so: */
                    if (fabs(Bjk_exp - Bjk_cmp) > tol[k])
                      { fprintf(stderr, "** (A X)[%d,%d] = %24.16e B[%d,%d] = %24.16e", i, k, , tol[k]);
