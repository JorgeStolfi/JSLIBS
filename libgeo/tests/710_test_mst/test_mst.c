/* Last edited on 2024-11-20 21:31:23 by stolfi */
...
void plot_mst (uint32_t N, r2_t st[], uint32_t P[], double C[], bool_t axes, char *prefix);
...
void draw_sites(epswr_figure_t *eps, uint32_t N, r2_t st[], uint32_t P[], double C[]);
...
int32_t main(int32_t argc, char **argv)
  { uint32_t N = (uint32_t)atoi(argv[1]);
    ...
    char *prefix = jsprintf("mst_%04d_%s_%s", N, distr_name, acost_name);
    ...
    auto double edist(uint32_t i, uint32_t j);         /* Euclidean distance between sites. */
    auto double qdist(uint32_t i, uint32_t j);         /* Euclidean distance with axes barriers. */
    ...
    double edist(uint32_t i, uint32_t j)
      { return r2_dist(&(st[i]), &(st[j])); }
    
    double qdist(uint32_t i, uint32_t j)
      { r2_t *si = &(st[i]);
        r2_t *sj = &(st[j]);
