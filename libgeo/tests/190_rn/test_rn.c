...
/* Last edited on 2024-11-08 12:41:56 by stolfi */
...
void test_rn_scale_shift_add_sub_mix(int32_t n, double a[], double b[], double c[], bool_t verbose);
...
void test_rmxn_add(int32_t m, int32_t n, double Amn[], double Bmn[], double Cmn[], bool_t verbose);
void test_rmxn_sub(int32_t m, int32_t n, double Amn[], double Bmn[], double Cmn[], bool_t verbose);
...
void test_rn (bool_t verbose)
  { double *a, *b, *c, *d;
    ...
    test_rn_scale_shift_add_sub_mix(n, a, b, c, verbose);
...
void test_rn_add_sub_neg(int32_t n, double a[], double b[], double c[], bool_t verbose)
  {
    ...
    for (int32_t i = 0; i < n; i++)
      { double dci = -a[i];
        rn_check_eq(c[i], dci, &i, NO, "rn_neg error"); }
 }
...
void test_rn_scale_shift_add_sub_mix(int32_t n, double a[], double b[], double c[], bool_t verbose)
  {
    ...
    /* TEST: void rn_add (int32_t n, double *a, double *b, double *c); */
    /* TEST: void rn_sub (int32_t n, double *a, double *b, double *r); */
    ...
    for (int32_t i = 0; i < n; i++)
      { double dci = s*a[i];
        ...
        rn_check_eq(c[i], dci, &i, NO, "rn_scale error(1)");
      }
    ...
    for (int32_t i = 0; i < n; i++)
      { double dci = a[i] + b[i];
        fprintf(stderr, "%5d %24.16e %24.16e %24.16e %24.16e\n", i, a[i], b[i], c[i], dci);
        rn_check_eq(c[i], dci, &i, NO, "rn_add error(1)");
      }
    ...
    for (int32_t i = 0; i < n; i++)
      { double dci = a[i] - b[i];
        fprintf(stderr, "%5d %24.16e %24.16e %24.16e %24.16e\n", i, a[i], b[i], c[i], dci);
        rn_check_eq(c[i], dci, &i, NO, "rn_sub error(1)");
      }
    ...
    for (int32_t i = 0; i < n; i++)
      { double dci = s * a[i] + t * b[i];
        rn_check_eq(c[i], dci, &i, NO, "rn_mix error");
      }
    ...
    for (int32_t i = 0; i < n; i++)
      { double dci = b[i] + s * a[i];
        rn_check_eq(c[i], dci, &i, NO, "rn_mix_in error");
      }
 }
...
void test_rn_weigh_unweigh(int32_t n, double a[], double b[], double c[], bool_t verbose)
  {
    ...
    for (int32_t i = 0; i < n; i++)
      { double dci = a[i] * b[i];
        rn_check_eq(c[i], dci, &i, NO, "rn_weigh error");
      }
    ...
    for (int32_t i = 0; i < n; i++)
      { double dci = a[i] / b[i];
        rn_check_eq(c[i], dci, &i, NO, "rn_unweigh error");
      }
 }
...
void test_rmxn_add(int32_t m, int32_t n, double Amn[], double Bmn[], double Cmn[], bool_t verbose)
  {
    /* TEST: void rmxn_add (int32_t m, int32_t n, double *A, double *B, double *M); */
    ...
    for (int32_t i = 0; i < m; i++)
      { for (int32_t j = 0; j < n; j++)
          { double zij = Amn[n*i + j] + Bmn[n*i + j];
            rn_check_eq(Cmn[n*i + j], zij, &i, &j, "rmxn_add error(1)");
          }
      }
  }
...
void test_rmxn_sub(int32_t m, int32_t n, double Amn[], double Bmn[], double Cmn[], bool_t verbose)
  {
    /* TEST: void rmxn_sub (int32_t m, int32_t n, double *A, double *B, double *M); */
    ...
    for (int32_t i = 0; i < m; i++)
      { for (int32_t j = 0; j < n; j++)
          { double zij = Amn[n*i + j] - Bmn[n*i + j];
            rn_check_eq(Cmn[n*i + j], zij, &i, &j, "rmxn_sub error(1)");
          }
      }
  }
...
void test_rmxn_perturb_unif(int32_t m, int32_t n, double Amn[], bool_t verbose)
  {
    /* TEST: void rmxn_perturb_unif(int32_t m, int32_t n, double pabs, double prel, double M[]); */
