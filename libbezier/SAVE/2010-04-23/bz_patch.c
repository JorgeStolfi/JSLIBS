/* See bezier.h */
/* Last edited on 2010-04-23 11:09:28 by stolfi */
...
bz_patch_rdim_t bz_patch_get_rdim(bz_patch_t *b)
  { assert(b->C.ds.na >= b->d);
    return b->C.ds.na - b->d;
  }
  
void bz_patch_get_degrees(bz_patch_t *b, bz_degree_t g[])
  { int i;
    for (i = 0; i < b->d; i++)
      { assert(b->C.ds.sz[i] >= 1);
        g[i] = b->C.ds.sz[i] - 1;
      }
  }

bz_patch_t bz_patch_new(bz_patch_ddim_t d, const bz_degree_t g[], bz_patch_rdim_t r, const ix_size_t rsz[])
  { /* fprintf(stderr, "+ bz_patch_new\n");  */
    /* Build the size array: */
    demand(d + r <= double_array_MAX_AXES, "too many indices in domain+range");
    ix_dim_t na = d + r;
    ix_size_t bsz[na];
    int i;
    for (i = 0; i < d; i++) { bsz[i] = g[i] + 1; }
    for (i = 0; i < r; i++) { bsz[d + i] = (rsz == NULL ? 1 : rsz[i]); }
    /* Compute total size {tsz} of control array: */
    bz_patch_t b;
    b.d = d;
    b.C = double_array_new(na, bsz, ix_order_L);
    /* fprintf(stderr, "- bz_patch_new\n");  */ 
    return b;
  }

bz_patch_t bz_patch_uniform_new(bz_patch_ddim_t d, bz_degree_t g, bz_patch_rdim_t r, const ix_size_t rsz[])
  { /* fprintf(stderr, "+ bz_patch_new\n");  */
    /* Build a uniform degree array: */
    bz_degree_t gg[d];
    int i;
    for (i = 0; i < d; i++) { gg[i] = g; }
    bz_patch_t b = bz_patch_new(d, gg, r, rsz);
    /* fprintf(stderr, "- bz_patch_new\n");  */ 
    return b;
  }
...
bz_patch_t bz_patch_facet_new(bz_patch_t *b, box_axis_index_t j)
  { bz_degree_t fg[bz_patch_MAX_DDIM];
    ...
    ix_size_t fsz[];
    int bi, fi;
    for (bi = 0, fi = 0; bi < d; bi++)
      { if (bi != j) { fg[fi] = g[bi]; fi++; } }
    return bz_patch_new(b->d-1, fg, );
  }
...
double *bz_patch_control_point(bz_patch_t *b, bz_patch_cpindex_t e[])
  { int ix, i;
    /* Compute index {ix} of point in point array, by the formula
      {ix = SUM{ PROD{(g[j]+1) : j=i+1..d-1}*e[i] :  i = 0..d-1}} */
    ix = 0; for (i = 0; i < b->d; i++) { ix = ix*(b->g[i]+1) + e[i]; }
    /* Return address of first coordinate; */
    return &(b->c[ix * b->n]);
  }

void bz_patch_eval(bz_patch_t *b, double x[], double sx[])
  { bz_patch_eval_m(b->d, b->n, b->g, b->c, x, sx); }
...
void bz_patch_compute_steps(
  ...
  )
  {
    int tsz = 1, i;
    bz_degree_t *g = &(b->g[0]);
    for (i = b->d-1; i >= 0; i--) { step[i] = tsz; tsz *= (g[i] + 1); }
    (*sz) = tsz;
  }
...
void bz_patch_raise_degree(bz_patch_t *b, bz_patch_t *t)
  { /* Assumption: Linear interpolation of the indices? */
    const bz_degree_t *bg = &(b->g[0]);
    const bz_degree_t *tg = &(t->g[0]);
    affirm(t->d == b->d, "domain dimension mismatch");
    affirm(t->n == b->n, "range dimension mismatch");
    { int i;
      for (i = 0; i < b->d; i++)
    ...
    { int m = b->d;
      ...
      for (tix = 0; tix < tsz; tix++)
        { /* Compute nominal position {x[0..d-1]} of control point {t->c[tix]}: */
          int tt = tix, i;
          for (i = m - 1; i >= 0; i--) 
      ...
      { int m = b->d;
        ...
        for (bix = 0; bix < bsz; bix++)
          { /* Compute nominal coordinates {x} of control point {b->c[bix]}: */
            ...
            /* Evaluate the multiaffine spline {t} at {x}: */
...
void bz_patch_enum_sub
  ...
  )
  {
    ...
    bz_patch_t t = bz_patch_uniform_new(b->d, b->n, 1);
    
    auto void recurse(bz_patch_t *b, interval_t box[], int rank);
      /* Performs the task on a sub-patch {b,box} at level {rank}. */
    
    void recurse(bz_patch_t *b, interval_t box[], int rank)
      { /* Decide whether a patch must be split or is a leaf: */
        bool_t leaf;
        if (rank < minRank)
          { leaf = FALSE; }
          ...
          { bz_patch_multiaffine_approx(b, &t);
            double err = bz_patch_multiaffine_error(b, &t);
            if (err <= tol)
              { leaf = TRUE; }
            else
              { leaf = FALSE; }
        ...
        if (leaf)
          ...
          { /* Split along axis {rank % b->d}: */
            box_axis_t a = rank % b->d;
            bz_patch_t bLO = bz_patch_new(b->d, b->n, b->g);
            bz_patch_t bHI = bz_patch_new(b->d, b->n, b->g);
            interval_t boxLO[b->d], boxHI[b->d];
            box_split(b->d, box, a, 0.5, boxLO, boxHI);
            bz_patch_split(b, a, 0.5, &bLO, &bHI);
            recurse(&bLO, boxLO, rank+1);
            recurse(&bHI, boxHI, rank+1);
            bz_patch_free(&bLO);
            bz_patch_free(&bHI);
          }
      }
...
void bz_patch_print(FILE *wr, bz_patch_t *b, char *fmt)
  {
    ...
    double *bc = &(b->c[0]);
    int n = b->n;
    { int i; bsz = 1; for (i = 0; i < b->d; i++) { bsz *= b->g[i]+1; } }
    rowsz = ( b->d >= 1 ? b->g[b->d-1]+1 : bsz + 1 );
    planesz = ( b->d >= 2 ? b->g[b->d-2]+1 : bsz + 1 );
    for (ix = 0; ix < bsz; ix++)
      { for (j = 0; j < n; j++,bc++)
...
bz_patch_t bz_patch_from_box(bz_patch_ddim_t d, interval_t box[])
  { 
    /* fprintf(stderr, "+ bz_patch_from_box\n"); */
