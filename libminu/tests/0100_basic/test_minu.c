// See test_minu.h
// Last edited on 2024-11-08 11:30:35 by stolfi
...
epswr_figure_t *tmnu_new_figure
  ( int32_t i_opt, 
    Minimizer *opt, 
    int32_t i_prb, 
    Problem *prb, 
...
bool_t tmnu_multiple_eval
  ( void *parms, /* Actually a TestParms */ 
    double x, double *fx, double *dfx
  );

bool_t tmnu_multiple_check 
  ( void *parms, /* Actually a TestParms */ 
    double a, double b,
    double x, double fx,
    double dfx, double dq, double ddfx, double ddq
...
Performance tmnu_single
  ...
  { epswr_figure_t *eps = tmnu_new_figure(i_opt, opt, i_prb, prb, "sngl");
    TestParms tp = (TestParms){prb, opt, eps, 0};
    fprintf(stderr, "\n");
    fprintf(stderr, "=============================================================================\n");
    fprintf(stderr, "===== tmnu_single(%s, %s)\n\n", opt->name, prb->name);
    ...
    tmnu_print_status(tp.nCalls, error, x, fx, dfx,1.0, 0.0,0.0);

    fprintf(stderr, "=============================================================================\n");
    fprintf(stderr, "\n");
    tmnu_end_figure(eps, tp.nCalls, error, opt, prb);
...
Performance tmnu_multiple 
  ...
  )
  {
    ...
    fprintf(stderr, "\n");
    fprintf(stderr, "=============================================================================\n");
    fprintf(stderr, "===== tmnu_multiple (%s,%s)\n\n", opt->name, prb->name);
    affirm(nTests >= 2, "");
...
bool_t tmnu_single_eval(void *parms, double x, double *fx, double *dfx)
  {
    TestParms *tp = (TestParms *)parms;
    Problem *prb = tp->prb;
    prb->eval(prb, x, fx, dfx);
    tmnu_plot_dot(tp->eps, prb,
...
bool_t tmnu_single_check
  ...
  )
  {
    ...
    tmnu_print_status(tp->nCalls, err, x, fx, dfx,dq, ddfx,ddq);
    tmnu_plot_dot(tp->eps, prb, x, fx, TRUE);
    return ((err <= 0.0) || (tp->nCalls >= prb->maxCalls));
  }

bool_t tmnu_multiple_eval(void *parms, double x, double *fx, double *dfx)
  {
    TestParms *tp = (TestParms *)parms;
    Problem *prb = tp->prb;
    prb->eval(prb, x, fx, dfx);
...
epswr_figure_t *tmnu_new_figure
  ...
  { char *fname = NULL;
    asprintf(&fname, "aumt_opt%02d_prb%02d_%s", i_opt, i_prb, tag);
    ...
    for (int32_t i = 1; i <= NSteps; i++)
      { double xa = xb; 
        ...
        xb = xb + dx; 
        prb->eval(prb, xb, &yb, &dfxb);
        tmnu_draw_segment(eps, prb, xa, ya, xb, yb);
      }
    free(fname);
    return eps;
  }
