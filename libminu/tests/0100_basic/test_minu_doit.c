...
/* Last edited on 2024-11-08 11:34:42 by stolfi  */
  ...
  for (int32_t i_opt = 0; i_opt < NOpt; i_opt++)
    { for (int32_t i_prb = 0; i_prb < NPrb; i_prb++) 
        { int32_t ij = NPrb*i_opt+i_prb;
          test_minu_single(i_opt, &(opt[i_opt]), i_prb, &(prb[i_prb]), TRUE);
          srandom(2567898753u);
          perf[ij] = test_minu_multiple(i_opt, &(opt[i_opt]), i_prb, &(prb[i_prb]), 100);
        }
    }
