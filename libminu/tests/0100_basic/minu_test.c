// See minu_test.h
// Last edited on 2023-02-19 16:51:53 by stolfi
...
epswr_figure_t *minu_new_figure
  ( int32_t i_opt, 
    Minimizer *opt, 
    int32_t i_prb, 
    Problem *prb, 
...
Performance minu_test_single
  ...
  { epswr_figure_t *eps = minu_new_figure(i_opt, opt, i_prb, prb, "sngl");
    ...
    fprintf(stderr, "=============================================================================\n");
    fprintf(stderr, "\n");
    minu_test_end_figure(eps, tp.nCalls, error, opt, prb);
...
bool_t minu_test_single_check
  ...
  )
  {
    ...
    minu_test_plot_dot(tp->eps, prb, x, fx, TRUE);
  ...
  { char *fname = NULL;
    ...
    for (int32_t i = 1; i <= NSteps; i++)
      { double xa = xb; 
        ...
        xb = xb + dx; 
        prb->eval(prb, xb, &yb, &dfxb);
        minu_test_draw_segment(eps, prb, xa, ya, xb, yb);
      }
    free(fname);
    return eps;
  }
