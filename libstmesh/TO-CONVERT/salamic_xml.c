...
#define salamic_xml_delete_node( node )	{			\
        if ( node ) {						\
...
#define salamic_xml_delete_attribute( attrib ) {		\
        if ( attrib ) {							\
...
XMLElement::~XMLElement()
{
    while( _rootAttribute ) {
        ...
        salamic_xml_delete_attribute( _rootAttribute );
        _rootAttribute = next;
    }
}
...
void XMLElement::DeleteAttribute( const char* name )
{
    ...
    for( XMLAttribute* a=_rootAttribute; a; a=a->_next ) {
        if ( XMLUtil::StringEqual( name, a->Name() ) ) {
            ...
            salamic_xml_delete_attribute( a );
            break;
        }
        prev = a;
    }
    ...
    while( p ) {
        ...
        if (XMLUtil::IsNameStartChar( *p ) ) {
            ...
            if ( !p || Attribute( attrib->Name() ) ) {
                salamic_xml_delete_attribute( attrib );
...
void XMLDocument::PrintError() const
{
    if ( _errorID ) {
        ...
        if ( _errorStr1 ) {
            salamic_xml_snprintf( buf1, LEN, "%s", _errorStr1 );
        }
        if ( _errorStr2 ) {
            salamic_xml_snprintf( buf2, LEN, "%s", _errorStr2 );
        }
