...
void hpmat_one(hpmat_options_t *o, char *method, bool_t verbose)
  { 
    ...
    auto bool_t ok_pred(hr2_pmap_t *A, double fA);
      /* Checks whether {A} is acceptable.
        Currently, that means {fA} is less than {max_mismatch}. */
    ...
    if (strcmp(method, "quad") == 0)
      { double tol = 0.02;
        hr2_pmap_opt_quadratic(type, mismatch_sqr, ok_pred, maxIter, &Asol, &F2sol);
      }
    else
