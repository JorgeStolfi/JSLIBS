...
Float compute_minw (Interval xv, Float epsilon, Float delta);
/*
   Returns the maximum length of a root interval whose lower endpoint is 
   $xlo$.  By definition, an interval $xv$ can be a root if and only if
...
void zerofind(
    ...
    int report (Interval xv, IntervalPair yv, void *data),
    void *data,
    Interval xd,
    Float epsilon,
    Float delta
  )
  {
    ...
    while (xv.lo <= xv.hi)
      {
        ...
        minw = compute_minw(xv, epsilon, delta);
        ...
        */
        ...
        if ( (xroot.lo <= xroot.hi) && (is_void || ( xv.lo > xroot.hi)) )
          { /* Flush current root: */
            int stop = report (xroot, yroot, data);
            if (stop) return; 
            xroot.lo = One;
            xroot.hi = Zero;
          };
    ...
    /* Flush last root: */
    if (xroot.lo <= xroot.hi) { (void) report(xroot, yroot, data); }
  }
...
Float compute_minw (Interval xv, Float epsilon, Float delta)
  { Float minw = delta;
    ...
    #if DEBUG
      fprintf(stderr,"compute_minw: xv = "); ia_print(stderr,xv);
    #endif
