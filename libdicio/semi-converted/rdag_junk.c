...
PROCEDURE ReadParam(rd: Rd.T; label: char *): NAT;
/*
...
PROCEDURE ReadParam(rd: Rd.T; label: char *): NAT ==
  ...
  {
    for (i = 0 TO Text.Length(label) - 1){
      with (c == Rd.GetChar(rd)){
        assert(c == Text.GetChar(label, i) );
      ;};
    ;};
    with (i == RdNat(rd), c == Rd.GetChar(rd)){
      assert(c == '\n' );
      return i
    ;};
  ;} ReadParam;
...
PROCEDURE Set(dag: T; s: State; r, w: Symbol; r, l: State) RAISES {Full} ==
  ...
  {
    if ((dag.e == NULL) || (s > LAST(dag.e^))){ RAISE Full ;};
    ...
    with (
      ...
      if ((l == s)){
        ...
        assert(e[l].w == w );
      ;};
      if ((s < oldNStates-1)  AND  AND  (oldClass!=w)){ 
...
FROM BinGraph.Copy
    ...
    if ((map == NULL) || (LAST(map^) < s)){
      with (
        newmap == NEW(REF ARRAY OF State, s + 1),
        nm == newmap^
     ){
        if ((map == NULL)){
          for (i = 0 TO LAST(nm)){ nm[i] = NoState ;};
        }else{
          with (m == map^){
            for (i = 0 TO LAST(m)){ nm[i] = m[i] ;};
            for (i = NUMBER(m) TO LAST(nm)){ nm[i] = NoState ;};
          ;}
        ;};
        map = newmap
      ;}
...
PROCEDURE SetRep(t:Ctypes.long): Time ==
  ...
  ;} SetRep;
    ...
    MonthName == ARRAY Date.Month OF char *{
