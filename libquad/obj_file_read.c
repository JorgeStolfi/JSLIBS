/* See {obj_file_read.h}. */
/* Last edited on 2024-12-05 10:39:17 by stolfi */
...
void obj_file_read_err(int32_t nlin, char *msg);
  /* Prints the line number {nlin} and the message {msg}, then bombs out. */

void obj_file_read_command(FILE *rd, int32_t *ch1_P, int32_t *ch2_P, int32_t *nlin_P);
...
int32_t obj_file_read_index(FILE *rd, int32_t nlin, char *elname, int32_t n); 
...
obj_file_data_t *obj_file_read(FILE *rd, bool_t verbose)
  { int32_t nlin = 0; /* Number of lines read. */
    int32_t nv = 0; /* Number of vertices ('v' lines) found. */
    int32_t nt = 0; /* Number of texpoints ('vt' lines) found. */
    int32_t nn = 0; /* Number of normals ('vn' lines) found. */
    int32_t nf = 0; /* Number of faces ('f' lines) found. */
    int32_t ns = 0; /* Number of sides/corners (entries in 'f' lines) found. */
      ...
      { int32_t ch1, ch2; /* The two characters of an OBJ line type. */
        ...
        if (ch1 == 'v')
          ...
          { /* Face definition line: */
            assert(ch2 == ' ');
            obj_file_face_vec_expand(&(D->FV), nf); D->FV.e[nf] = int32_vec_new(5); 
            obj_file_face_vec_expand(&(D->FT), nf); D->FT.e[nf] = int32_vec_new(5); 
            obj_file_face_vec_expand(&(D->FN), nf); D->FN.e[nf] = int32_vec_new(5); 
...
void  obj_file_read_face
  ...
  { int32_t nc = 0;  /* Number of corners read for this face. */
    while(TRUE)
      { if (debug) { fprintf(stderr, "      looking for corner data, nc = %d\n", nc); }
        ...
        int32_vec_expand(FVk, nc); 
        int32_vec_expand(FTk, nc); 
        int32_vec_expand(FNk, nc); 
