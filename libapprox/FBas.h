...
typedef double FBas_EvalMth(OBJ *bas, int32_t index, int32_t dp, double *p);
  /* A method that evaluates the basis element {bas[index]}
    at the point {p[0..dp-1]}. */

typedef double FBas_EnergyMth(OBJ *bas, int32_t i, int32_t j);
  /* A method that returns the entry {[i,j]} of the matrix that
    describes a quadratic "internal energy" functional associated
    with basis {bas}. */

typedef int32_t FBas_RoundSizeMth(OBJ *bas, int32_t n, bool_t up);
