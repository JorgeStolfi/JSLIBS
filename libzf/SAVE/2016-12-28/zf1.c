/* See {zf1.h} */
/* Last edited on 2016-12-28 21:15:51 by stolfilocal */
...
typedef struct zf1_stack_entry_t {
    Interval xr;
    Interval yr;
    zf1_kind_t kind;
    int level;
...
Float zf1_compute_max_root_hi (Float xlo, Float epsilon, Float delta);
...
bool_t zf1_output_interval
  ...
  /* Feeds a new interval {x_this} into the interval filter. Namely,
    ...
    {y_prev}, and {k_prev} (if {k_prev} is not {zf1_kind_undefined});
...
void zf1_split_and_stack_trapezoid
  ( ia_trapez_t *tr,
    Interval yr,
    int level,
    zf1_stack_entry_t **topp,  /* Top-of-stack entry */
  ...
  /* Pushes back a trapezoid onto the stack, whole or in pieces.
    ...
    classification (either {zf1_kind_positive}, {zf1_kind_negative}, or
    {zf1_kind_undefined}).
...
zf1_kind_t zf1_enum_zeros
  ...
  { int nst = 0;                            /* Count of stacked intervals */
    zf1_stack_entry_t stack[STACKSIZE];      /* Stacked intervals */
    zf1_stack_entry_t *top = &(stack[0])-1;  /* The top-of-stack entry */

    Interval x_this = xd;         /* The current interval */
    Interval y_this = ia_full();  /* The range of defined values of F in {x_this} */

    zf1_kind_t k_this = zf1_kind_mixed;  /* The kind of {x_this}. */
    ...
    while (x_this.lo <= x_this.hi)
      {
        /* Current interval invariant:
           /\ ((x \in x_this) /\ (F(x) != undefined)) =>
                  /\ F(x) \in y_this
                  /\ (k_this == zf1_kind_positive) => F(x) > 0
                  /\ (k_this == zf1_kind_negative) => F(x) < 0
                  /\ k_this != zf1_kind_undefined
                  /\ k_this != zf1_kind_root
        */
        ...
        */
        ...
        /* filter invariant:
            /\ (x_prev.lo <= x_prev.hi) => 
                  /\ (x_prev.hi == x_this.lo)
                  /\ (k_prev != zf1_kind_undefined) => (y_prev.lo <= y_prev.hi)
            /\ (x_prev.lo > x_prev.hi) =>
                  /\ (xd.lo == x_this.lo))
        */

        if (k_this != zf1_kind_mixed)
          { /* Output this interval */
            bool_t stop = zf1_output_interval
          ...
          { /* Try to resolve the kind of {x_this} by evaluating {F} on it: */
            ...
            Float mrhi_this = zf1_compute_max_root_hi (x_this.lo, epsilon, delta);
...
#           if DEBUG
              ...
              fprintf(stderr, "  bits = %6.2f\n", 
                zf1_bits_gained(x_this.hi - x_this.lo, xd.hi - xd.lo)
              );
#           endif
...
#           endif
            ...
            k_this = zf1_classify_interval(&x_this, &y_this, mrhi_this);
            if (k_this != zf1_kind_mixed)
              { /* This interval cannot or need not be subdivided, ignore the butterfly. */
                ...
                zf1_stack_interval(&x_this, &y_this, k_this, l_this, &top, &nst);
              }
            else 
              { /* Split {x_this} into smaller pieces, if possible, and stack them. */
                ...
                if (xm == x_this.lo)
                  { /* Butterfly is just the hi trapezoid, split it: */
                    ...
                    zf1_split_and_stack_trapezoid(&(bt.tp[1]), y_this, l_this, &top, &nst);
                  }
                  ...
                  { /* Butterfly is just the lo trapezoid, split it: */
                    ...
                    zf1_split_and_stack_trapezoid(&(bt.tp[0]), y_this, l_this, &top, &nst);
                  }
                else
                  { /* Separate the butterfly into two trapezoids, stack them: */
                    ...
                    Float mrhi_b = zf1_compute_max_root_hi(xb.lo, epsilon, delta);
                    Interval ybt1 = ia_join(bt.tp[1].yxlo, bt.tp[1].yxhi);
                    Interval yb = ia_meet(y_this, ybt1);
                    zf1_kind_t kb = zf1_classify_interval(&xb, &yb, mrhi_b);
                    zf1_stack_interval(&xb, &yb, kb, l_this+1, &top, &nst);

                    /* Stack lo trapezoid: */
                    Interval xa = (Interval){x_this.lo, xm};
                    Float mrhi_a = zf1_compute_max_root_hi (xa.lo, epsilon, delta);
                    Interval ybt0 = ia_join(bt.tp[0].yxlo, bt.tp[0].yxhi);
                    Interval ya = ia_meet(y_this, ybt0);
                    zf1_kind_t ka = zf1_classify_interval(&xa, &ya, mrhi_a);
                    zf1_stack_interval(&xa, &ya, ka, l_this+1, &top, &nst);
                  }
              }
          }
...
zf1_kind_t zf1_classify_interval
  ...
  { if (yr->lo > yr->hi)
      ...
      { if (yr->lo > Zero)
          { return(zf1_kind_positive); }
        else if (yr->hi < Zero)
          { return(zf1_kind_negative); }
        else if ((yr->lo == Zero) && (yr->hi == Zero))
          { return(zf1_kind_root); }
        else
...
void zf1_split_and_stack_trapezoid
  ...
  { /* Shrink {yr} ito tight-fit {tr}: */
    ...
    zf1_kind_t kt = zf1_kind_mixed; /* For now. */
    if (yr.lo > yr.hi)
      { kt = zf1_kind_undefined; }
    else if (yr.lo > 0)
      { kt = zf1_kind_positive; }
    else if (yr.hi < 0)
      { kt = zf1_kind_negative; }
    
    if (kt != zf1_kind_mixed)
      { /* There are no zeros in this interval, so we can stack it whole. */
        zf1_stack_interval(&(tr->x), &yr, kt, level, topp, nstp);
      }
    else
      { /* At this point the trapezoid {tr} includes zero, so it must be split. */
        affirm ((yr.lo <= Zero) && (yr.hi >= Zero), "zf1_split_trapezoid: bad interval");
        Interval xm, ym;
        int l_new = level+1;
        Float xsplit;
        
        zf1_refine_interval(tr, &xm, &ym);
...
#     endif
        ...
        if (xm.hi < tr->x.hi) 
          { Interval xh, yh;
            zf1_kind_t kh = zf1_kind_undefined;
            xh.lo = xm.hi;
            xh.hi = tr->x.hi;
            if (tr->yxhi.lo > Zero)
              { kh = zf1_kind_positive;
                yh.lo = Zero;
                yh.hi = yr.hi;
              }
            else if (tr->yxhi.hi < Zero)
              { kh = zf1_kind_negative;
                yh.lo = yr.lo;
                yh.hi = Zero;
              }
            else
              { fatalerror("zf1_split_trapezoid: bad split"); }

            zf1_stack_interval(&xh, &yh, kh, l_new, topp, nstp);
          }
        ...
        if ((xsplit > xm.lo) && (xsplit < xm.hi))
          { /* Split middle piece: */
            ...
            zf1_stack_interval(&xt, &ym, zf1_kind_mixed, l_new, topp, nstp);
            xt.lo = xm.lo; xt.hi = xsplit;
            zf1_stack_interval(&xt, &ym, zf1_kind_mixed, l_new, topp, nstp);
          }
        else
          { /* Middle piece shrunk enough, stack it whole: */
            zf1_stack_interval(&xm, &ym, zf1_kind_mixed, l_new, topp, nstp);
          }

        /* Stack low-end piece: */
        if (tr->x.lo < xm.lo) 
          { Interval xl, yl;
            zf1_kind_t kl = zf1_kind_undefined;
            xl.lo = tr->x.lo;
            xl.hi = xm.lo;
            if (tr->yxlo.lo > Zero)
              { kl = zf1_kind_positive;
                yl.lo = Zero;
                yl.hi = yr.hi;
              }
            else if (tr->yxlo.hi < Zero)
              { kl = zf1_kind_negative;
                yl.lo = yr. lo;
                yl.hi = Zero;
              }
            else
              { fatalerror("zf1_split_trapezoid: bad split"); }
            zf1_stack_interval(&xl, &yl, kl, l_new, topp, nstp);
          }
      }
  }

void zf1_refine_interval(ia_trapez_t *tr, Interval *xm, Interval *ym)
  {
    /* Compute xm = sub-inderval of xv where yr may be zero: */

    /* Compute low end {xm->lo} of interval: */
    if (tr->yxlo.hi < Zero)
      { affirm(tr->yxhi.hi >= Zero, "zf1_refine_interval: all negative");
        xm->lo = flt_interp_lo(tr->yxlo.hi, tr->x.lo, tr->yxhi.hi, tr->x.hi, 0);
      }
    else if (tr->yxlo.lo > Zero)
      { affirm(tr->yxhi.lo <= Zero, "zf1_refine_interval: all positive");
    ...
    if (tr->yxhi.hi < Zero)
      { affirm(tr->yxlo.hi >= Zero, "zf1_refine_interval: all negative");
        xm->hi = flt_interp_hi(tr->yxhi.hi, tr->x.hi, tr->yxlo.hi, tr->x.lo, 0);
      }
    else if (tr->yxhi.lo > Zero)
      { affirm(tr->yxlo.lo <= Zero, "zf1_refine_interval: all positive");
...
void zf1_stack_interval
  ...
  { zf1_stack_entry_t *top = *topp;
    int nst = *nstp;
    if (nst > 0)
      { if ((kind == top->kind) && (kind != zf1_kind_mixed))
          { /* Merge {xr} with top-of-stack interval */
            top->xr.lo = xr->lo;
            if (kind != zf1_kind_undefined) 
              { if (yr->lo < top->yr.lo) top->yr.lo = yr->lo;
                if (yr->hi > top->yr.hi) top->yr.hi = yr->hi;
              }
            affirm(top->level <= level, "zf1_stack_interval: decreasing levels");
            return;
          }
      }
...
Float zf1_compute_max_root_hi (Float lo, Float epsilon, Float delta)
  { Float hi;
...
#   if DEBUG
      fprintf(stderr, "zf1_compute_max_root_hi: lo = %e  hi = %e \n", lo, hi);
#   endif
    
    affirm(hi > lo, "zf1_compute_max_root_hi: epsilon and delta are too small");
...
float zf1_bits_gained(Float curw, Float orgw)
  { float curbits, orgbits;
    affirm (curw <= orgw, "zf1_bits_gained: bad arguments");
    ROUND_NEAR;
    if (orgw == Zero) 
      { return(Zero); }
    else
