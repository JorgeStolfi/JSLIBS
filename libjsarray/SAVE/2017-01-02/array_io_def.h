...
/* Last edited on 2013-05-24 04:18:28 by stolfilocal */
...
#define array_IMPLEMENT_write(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
  array_DECLARE_write(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
    { ix_descr_t *DA = &(A->ds); \
      ...
      array_write_header(wr, #ARRAY_TYPE, NULL, DA, ixor, dix); \
      ix_index_t ix[na]; \
      bool_t first = TRUE; \
          ...
          do { \
            if (! first) \
              { /* Print one blank line for eack trailing 0 in {ix[0..na-1]}: */ \
                ia = ix_sync_level(na,DA->sz,ix,ixor,FALSE);            \
            ...
            fprintf(wr, "\n"); \
            first = FALSE; \
          } while (! ix_descr_next(DA,ix,ixor,&pA)); \
        } \
...
#define array_IMPLEMENT_read(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
  array_DECLARE_read(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
    { /* Allocate and read the array descriptor: */ \
      ...
      ix_descr_t *DA = &(A->ds); \
      ix_order_t ixor; \
      array_read_header(rd, #ARRAY_TYPE, NULL, DA, &ixor); \
      ix_dim_t na = DA->na; \
      int ia; \
      ...
      if (ix_descr_indices_first(DA, ix)) \
        { ix_pos_t p = ix_descr_position(DA, ix); \
          do { \
            ...
            fget_eol(rd); \
          } while (! ix_descr_next(DA,ix,ixor,&p)); \
        } \
