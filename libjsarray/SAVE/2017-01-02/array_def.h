...
/* Last edited on 2011-09-26 11:51:20 by stolfilocal */
...
#define array_DEFINE_BASIC_TYPES(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
  ...
  typedef ix_pos_count_t PREFIX##_count_t; \
...
#define array_DECLARE_new(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
  ARRAY_TYPE PREFIX##_new ( ix_dim_t na, ix_size_t sz[], ix_order_t ixor )

#define array_DECLARE_copy(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
  ARRAY_TYPE PREFIX##_copy( ARRAY_TYPE *A, ix_order_t ixor )
...
#define array_DECLARE_assign(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
  void PREFIX##_assign(ARRAY_TYPE *A, ARRAY_TYPE *B, ix_order_t ixor )
...
#define array_IMPLEMENT_new(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
  array_DECLARE_new(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
    { ARRAY_TYPE A; \
      ix_descr_t *DA = &(A.ds); \
      (*DA) = ix_descr_from_sizes(na, sz, ixor);            \
...
#define array_IMPLEMENT_copy(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
  array_DECLARE_copy(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
    { ARRAY_TYPE C = PREFIX##_new(A->ds.na, A->ds.sz, ixor); \
      /* !!! Should check for zero steps !!! */ \
      PREFIX##_assign(&C, A, ixor);                  \
      return C; \
    }
  ...
  array_DECLARE_assign(ARRAY_TYPE,PREFIX,ELEM_TYPE) \
    { auto void assign_elem ( const ix_index_t ix[], ix_pos_t pA, ix_pos_t pB, ix_pos_t pC ); \
      void assign_elem ( const ix_index_t ix[], ix_pos_t pA, ix_pos_t pB, ix_pos_t pC ) \
        { A->e[pA] = B->e[pB]; } \
      ix_descr_enum(&assign_elem, ixor, FALSE, &(A->ds), &(B->ds), NULL); \
     }
