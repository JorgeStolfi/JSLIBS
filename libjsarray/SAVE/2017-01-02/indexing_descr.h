...
/* Last edited on 2011-09-26 11:40:45 by stolfilocal */ 
...
ix_descr_t ix_descr_from_sizes ( ix_dim_t na, const ix_size_t sz[], ix_order_t ixor );
  /* Returns a descrptor record {D} for an array that has size {sz[i]}
    ...
    positions, according to the index variation order {ixor}. */
...
ix_pos_count_t ix_descr_num_tuples ( ix_descr_t *D );
...
ix_pos_count_t ix_descr_num_positions ( ix_descr_t *D );
...
bool_t ix_descr_next ( ix_descr_t *D, ix_index_t ix[], ix_order_t ixor, ix_pos_t *p );
  /* Increments the index tuple {ix} to the next valid tuple in the
    specified indexing order.
    
    More precisely, the procedure scans the indices {ix[0]} to
    {ix[D.na-1]}, in the specified order, looking for an index {ix[i]}
...
bool_t ix_descr_prev ( ix_descr_t *D, ix_index_t ix[], ix_order_t ixor, ix_pos_t *p );
  /* Like {ix_descr_next}, but in the reverse order. 
    
    More precisely, the procedure scans the indices {ix[0]} to
    {ix[D.na-1]}, in the order specified by {ixor}, looking for an index
...
sign_t ix_descr_compare ( ix_descr_t *D, const ix_index_t ixa[], const ix_index_t ixb[], ix_order_t ixor );
