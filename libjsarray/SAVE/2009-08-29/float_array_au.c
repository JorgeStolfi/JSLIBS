...
/* Last edited on 2009-08-31 11:22:26 by stolfi */
      ...
      { double *srow = snd->sv[s];
        for (c = 0; c < NC; c++)
          { /* Convert int sample {*srow} to float {v}, store, keep stats: */
            ...
            float_array_set_element(&A, ix, osmp);
            srow++;
          }
      }
...
sound_t *float_array_to_sound(float_array_t *A, int chns, int ch[], double freq, bool_t verbose)
  { 
    ...
    int na = D->na;
    assert(na == 2);
    int NS = D->sz[1];  /* Num of samples per channel. */
    ...
    for(s = 0; s < NS; s++)
      { double *srow = snd->sv[s];
        ...
        for (k = 0; k < sNC; k++)
          { c = (ch == NULL ? k : ch[k]);
            ix[0] = c; ix[1] = s; ix[2] = ix[3] = ix[4] = ix[5] = 0;
            float ismp = ((c < 0) || (c >= fNC) ? 0.0 : float_array_get_element(A, ix));
