...
#define PROG_DESC "checks the {psgr.h} routines"
#define PROG_VERS "1.0"

/* Last edited on 2025-04-27 02:55:44 by stolfi */
...
#define PROG_INFO_DESC \
  "  The program checks the functions {psgr_from_slope_map}.\n" \
  "\n" \
...
psgr_t* make_graph(float_image_t *G, bool_t extrapolate, bool_t addDiags);
  /* Creates a graph from the slope map {G}. */

void write_graph(char *prefix, psgr_t *gr);
  /* Writes {gr} to file "out/{prefix}-graph.txt". */
 
void plot_graph(char *prefix, psgr_t *gr);
  /* Writes an Encapsulated Postscript plot of {gr} to
    file "out/{prefix}-graph.eps". */
 
psgr_t* read_graph(char *prefix);
...
int32_t main(int32_t argc, char **argv)
  {
   /* Parse the command line options: */
    ...
    bool_t add_diags = FALSE; /* !!! Should be parameter !!! */
    psgr_t *gr = make_graph(G, extrapolate, add_diags);
...
psgr_t* make_graph(float_image_t *G, bool_t extrapolate, bool_t add_diags)
  { psgr_t *gr = psgr_from_slope_map(G, extrapolate, add_diags);
    return gr;
  }
...
void write_graph(char *prefix, psgr_t *gr)
  { char *fname = jsprintf("out/%s-graph.txt", prefix);
    bool_t verbose = TRUE;
    psgr_write_named(fname, gr, verbose);
    free(fname);
  }
...
psgr_t* read_graph(char *prefix)
  { char *fname = jsprintf("out/%s-graph.txt", prefix);
    bool_t verbose = TRUE;
    psgr_t *gr = psgr_read_named(fname, verbose);
    free(fname);
    return gr;
  }
...
void plot_graph(char *prefix, psgr_t *gr)
  { char *fname = jsprintf("out/%s-graph.eps", prefix);
    ...
    double vertexRadius = 0.50; /* Radius of vertices (mm) */
    double pixelSize = 5.0;     /* Image pixel size (mm). */
    psgr_plot_named(fname, gr, pixelSize, fontSize, vertexRadius); 
  }
