/* See {pst_gr.h} */
/* Last edited on 2025-03-11 06:40:28 by stolfi */
...
#include <pst_gr_integrate_iterative.h>
...
void pst_gr_integrate_iterative
  ...
  { int32_t indent = (level < -1 ? 0 : 2*level+2);
    ...
    int32_t *zid_from_vid = talloc(gr->NV, int32_t);
    pst_imgsys_t *S = pst_gr_integrate_build_system(gr, zid_from_vid, verbose);
    uint32_t NZ = S->N;
