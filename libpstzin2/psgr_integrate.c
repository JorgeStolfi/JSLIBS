/* See {psgr.h} */
/* Last edited on 2025-04-27 02:54:38 by stolfi */
...
pst_imgsys_t *psgr_integrate_build_system
  ...
  { 
    ...
    for (uint32_t kv = 0; kv < gr->NV ; kv++)
      { psgr_vertex_data_t *vdk = &(gr->vdata[kv]);
        if (???psgr_vertex_mark(vdk???) == DELETED) 
      ...
      { int32_t kz = kz_from_kv[kv];
        if (kz != -1)
          { assert((kz >= 0) && (kz < NZ));
            ...
            psgr_arc_t a0 = vdk->aout;
            psgr_arc_t aj = a0;
            do
              { double dj = psgr_arc_delta(gr, aj);
                double wj = psgr_arc_weight(gr, aj);
                uint32_t dstj = psgr_arc_org(gr,psgr_arc_sym(aj)); /* Index of {aj} dst vertex. */
                assert(dstj < gr->NV);
