...
pst_imgsys_t *pst_img_graph_integrate_build_system
  ...
  { 
    ...
    for (uint32_t kv = 0; kv < g->NV ; kv++)
      { pst_img_graph_vertex_data_t *vdk = &(g->vdata[kv]);
      ...
      { int32_t kz = kz_from_kv[kv];
        if (kz != -1)
          { assert((kz >= 0) && (kz < NZ));
            ...
            haf_arc_t a0 = vdk->aout;
            haf_arc_t aj = a0;
            do
              { double dj = pst_img_graph_get_arc_delta(g, aj);
                double wj = pst_img_graph_get_edge_weight(g, aj);
                uint32_t dstj = pst_img_graph_get_arc_origin(g,haf_sym(aj)); /* Index of {aj} dst vertex. */
                assert(dstj < g->NV);
    ...
    assert(N_gone + N_null + N_good == g->NV);
